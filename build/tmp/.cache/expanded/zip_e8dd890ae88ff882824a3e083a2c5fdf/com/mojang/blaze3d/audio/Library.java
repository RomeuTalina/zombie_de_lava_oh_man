...
public class Library {
    ...
    public void init(@Nullable String pDeviceSpecifier, boolean pEnableHrtf) {
        ...
        if (OpenAlUtil.checkALCError(this.currentDevice, "Get capabilities")) {
            ...
            try (MemoryStack memorystack = MemoryStack.stackPush()) {
                IntBuffer intbuffer = this.createAttributes(memorystack, alccapabilities.ALC_SOFT_HRTF && pEnableHrtf);
    ...
    private IntBuffer createAttributes(MemoryStack pMemoryStack, boolean pEnableHrtf) {
        int i = 5;
        IntBuffer intbuffer = pMemoryStack.callocInt(11);
        int j = ALC10.alcGetInteger(this.currentDevice, 6548);
        if (j > 0) {
            intbuffer.put(6546).put(pEnableHrtf ? 1 : 0);
