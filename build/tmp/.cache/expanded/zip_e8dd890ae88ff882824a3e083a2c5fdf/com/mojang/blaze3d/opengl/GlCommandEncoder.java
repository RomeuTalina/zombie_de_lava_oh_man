...
public class GlCommandEncoder implements CommandEncoder {
    ...
    private void verifyColorTexture(GpuTexture pTexture) {
        if (!pTexture.getFormat().hasColorAspect()) {
            throw new IllegalStateException("Trying to clear a non-color texture as color");
        } else if (pTexture.isClosed()) {
            throw new IllegalStateException("Color texture is closed");
        } else if ((pTexture.usage() & 8) == 0) {
            throw new IllegalStateException("Color texture must have USAGE_RENDER_ATTACHMENT");
        } else if (pTexture.getDepthOrLayers() > 1) {
            throw new UnsupportedOperationException("Clearing a texture with multiple layers or depths is not yet supported");
        }
    }

    private void verifyDepthTexture(GpuTexture pTexture) {
        if (!pTexture.getFormat().hasDepthAspect()) {
    ...
    protected <T> void executeDrawMultiple(
        ...
        @Nullable GpuBuffer pBuffer,
        @Nullable VertexFormat.IndexType pIndexType,
        Collection<String> pUniforms,
        T pData
    ) {
        if (this.trySetup(pRenderPass, pUniforms)) {
            if (pIndexType == null) {
                pIndexType = VertexFormat.IndexType.SHORT;
            }

            for (RenderPass.Draw<T> draw : pDraws) {
                VertexFormat.IndexType vertexformat$indextype = draw.indexType() == null ? pIndexType : draw.indexType();
                pRenderPass.setIndexBuffer(draw.indexBuffer() == null ? pBuffer : draw.indexBuffer(), vertexformat$indextype);
                pRenderPass.setVertexBuffer(draw.slot(), draw.vertexBuffer());
                ...
                if (biconsumer != null) {
                    biconsumer.accept(pData, (p_404740_, p_404741_) -> {
                        if (pRenderPass.pipeline.program().getUniform(p_404740_) instanceof Uniform.Ubo(int i)) {
                            GL32.glBindBufferRange(35345, i, ((GlBuffer)p_404741_.buffer()).handle, p_404741_.offset(), p_404741_.length());
                        }
                    });
                }

                this.drawFromBuffers(pRenderPass, 0, draw.firstIndex(), draw.indexCount(), vertexformat$indextype, pRenderPass.pipeline, 1);
            }
        }
    }
    ...
    protected void executeDraw(GlRenderPass pRenderPass, int pFirstIndex, int pIndex, int pIndexCount, @Nullable VertexFormat.IndexType pIndexType, int pPrimCount) {
        if (this.trySetup(pRenderPass, Collections.emptyList())) {
            if (GlRenderPass.VALIDATION) {
                if (pIndexType != null) {
                    if (pRenderPass.indexBuffer == null) {
                        throw new IllegalStateException("Missing index buffer");
                    ...
                    if ((pRenderPass.indexBuffer.usage() & 64) == 0) {
                ...
                if ((pRenderPass.vertexBuffers[0].usage() & 32) == 0) {
                    throw new IllegalStateException("Vertex buffer must have GpuBuffer.USAGE_VERTEX!");
                }
            }

            this.drawFromBuffers(pRenderPass, pFirstIndex, pIndex, pIndexCount, pIndexType, pRenderPass.pipeline, pPrimCount);
        }
    }
            ...
            if (pPrimCount > 1) {
                if (pFirstIndex > 0) {
                    GL32.glDrawElementsInstancedBaseVertex(
                        GlConst.toGl(pPipeline.info().getVertexFormatMode()),
                        pIndexCount,
                        GlConst.toGl(pIndexType),
                        (long)pIndex * pIndexType.bytes,
                        pPrimCount,
                        pFirstIndex
                    );
                } else {
                ...
                GL32.glDrawElementsBaseVertex(
                    GlConst.toGl(pPipeline.info().getVertexFormatMode()), pIndexCount, GlConst.toGl(pIndexType), (long)pIndex * pIndexType.bytes, pFirstIndex
                );
            } else {
                GlStateManager._drawElements(
                    GlConst.toGl(pPipeline.info().getVertexFormatMode()), pIndexCount, GlConst.toGl(pIndexType), (long)pIndex * pIndexType.bytes
                );
            }
        } else if (pPrimCount > 1) {
            GL31.glDrawArraysInstanced(GlConst.toGl(pPipeline.info().getVertexFormatMode()), pFirstIndex, pIndexCount, pPrimCount);
        } else {
            GlStateManager._drawArrays(GlConst.toGl(pPipeline.info().getVertexFormatMode()), pFirstIndex, pIndexCount);
        }
    }

    private boolean trySetup(GlRenderPass pRenderPass, Collection<String> pUniforms) {
        if (GlRenderPass.VALIDATION) {
            ...
            for (RenderPipeline.UniformDescription renderpipeline$uniformdescription : pRenderPass.pipeline.info().getUniforms()) {
                GpuBufferSlice gpubufferslice = pRenderPass.uniforms.get(renderpipeline$uniformdescription.name());
                if (!pUniforms.contains(renderpipeline$uniformdescription.name())) {
            ...
            for (Entry<String, Uniform> entry1 : pRenderPass.pipeline.program().getUniforms().entrySet()) {
                if (entry1.getValue() instanceof Uniform.Sampler) {
                    String s1 = entry1.getKey();
                    GlTextureView gltextureview = (GlTextureView)pRenderPass.samplers.get(s1);
        ...
        for (Entry<String, Uniform> entry : glprogram.getUniforms().entrySet()) {
            ...
            boolean flag = pRenderPass.dirtyUniforms.contains(s);
            switch ((Uniform)entry.getValue()) {
                case Uniform.Ubo(int j2):
                    int k = j2;
                    if (flag) {
                        GpuBufferSlice gpubufferslice1 = pRenderPass.uniforms.get(s);
                ...
                case Uniform.Utb(int l, int i1, TextureFormat textureformat, int i2):
                    ...
                    if (flag) {
                        GpuBufferSlice gpubufferslice2 = pRenderPass.uniforms.get(s);
                ...
                case Uniform.Sampler(int $$22, int l1):
                    int k1 = l1;
                    GlTextureView gltextureview1 = (GlTextureView)pRenderPass.samplers.get(s);
        ...
        if (pRenderPass.isScissorEnabled()) {
            GlStateManager._enableScissorTest();
            GlStateManager._scissorBox(pRenderPass.getScissorX(), pRenderPass.getScissorY(), pRenderPass.getScissorWidth(), pRenderPass.getScissorHeight());
        } else {
            GlStateManager._disableScissorTest();
        }
