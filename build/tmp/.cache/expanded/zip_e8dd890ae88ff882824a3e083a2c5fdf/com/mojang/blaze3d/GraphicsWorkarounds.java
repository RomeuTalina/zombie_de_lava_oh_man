...
public class GraphicsWorkarounds {
    ...
    private GraphicsWorkarounds(GpuDevice pDevice) {
        this.gpuDevice = new WeakReference<>(pDevice);
        this.alwaysCreateFreshImmediateBuffer = isIntelGen11(pDevice);
    }

    public static GraphicsWorkarounds get(GpuDevice pDevice) {
        GraphicsWorkarounds graphicsworkarounds = instance;
        if (graphicsworkarounds == null || graphicsworkarounds.gpuDevice.get() != pDevice) {
            instance = graphicsworkarounds = new GraphicsWorkarounds(pDevice);
        }
    ...
    private static boolean isIntelGen11(GpuDevice pDevice) {
        String s = GLX._getCpuInfo().toLowerCase(Locale.ROOT);
        String s1 = pDevice.getRenderer().toLowerCase(Locale.ROOT);
