...
public abstract class BufferStorage {
    public static BufferStorage create(GLCapabilities pCapabilities, Set<String> pEnabledExtensions) {
        if (pCapabilities.GL_ARB_buffer_storage && GlDevice.USE_GL_ARB_buffer_storage) {
            pEnabledExtensions.add("GL_ARB_buffer_storage");
    ...
    static class Immutable extends BufferStorage {
        ...
        @Nullable
        private ByteBuffer tryMapBufferPersistent(DirectStateAccess pDirectStateAccess, int pUsage, int pHandle, int pSize) {
            int i = 0;
            if ((pUsage & 1) != 0) {
                i |= 1;
            }
            ...
            if (i != 0) {
                ...
                bytebuffer = pDirectStateAccess.mapBufferRange(pHandle, 0, pSize, i | 64);
                if (bytebuffer == null) {
