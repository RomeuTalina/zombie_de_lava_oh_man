...
public class GetServerDetailsTask extends LongRunningTask {
    ...
    public RealmsLongRunningMcoTaskScreen connectScreen(RealmsJoinInformation pServerAddress) {
        return new RealmsLongRunningMcoConnectTaskScreen(this.lastScreen, pServerAddress, new ConnectTask(this.lastScreen, this.server, pServerAddress));
    }

    private PopupScreen resourcePackDownloadConfirmationScreen(RealmsJoinInformation pServerAddress, UUID pPackId, Function<RealmsJoinInformation, Screen> pConnectScreenSupplier) {
        ...
        return RealmsPopups.infoPopupScreen(this.lastScreen, component, p_404766_ -> {
            ...
            this.scheduleResourcePackDownload(pServerAddress, pPackId).thenRun(() -> setScreen(pConnectScreenSupplier.apply(pServerAddress))).exceptionally(p_287306_ -> {
    ...
    private CompletableFuture<?> scheduleResourcePackDownload(RealmsJoinInformation pServerAddress, UUID pPackId) {
        try {
            if (pServerAddress.resourcePackUrl() == null) {
                ...
                CompletableFuture<Void> completablefuture = downloadedpacksource.waitForPackFeedback(pPackId);
                downloadedpacksource.allowServerPacks();
                downloadedpacksource.pushPack(pPackId, new URL(pServerAddress.resourcePackUrl()), pServerAddress.resourcePackHash());
