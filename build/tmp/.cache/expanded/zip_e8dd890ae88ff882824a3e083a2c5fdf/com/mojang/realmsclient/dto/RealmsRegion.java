...
public enum RealmsRegion {
    ...
    private RealmsRegion(final String pNameId, final String pTranslationKey) {
        this.nameId = pNameId;
        this.translationKey = pTranslationKey;
    }

    @Nullable
    public static RealmsRegion findByNameId(String pNameId) {
        for (RealmsRegion realmsregion : values()) {
            if (realmsregion.nameId.equals(pNameId)) {
                return realmsregion;
            }
        }
    ...
    public static class RealmsRegionJsonAdapter extends TypeAdapter<RealmsRegion> {
        ...
        public void write(JsonWriter pWriter, RealmsRegion pRegion) throws IOException {
            pWriter.value(pRegion.nameId);
        }

        public RealmsRegion read(JsonReader pReader) throws IOException {
