...
public interface RealmsError {
    ...
    static RealmsError parse(int pHttpCode, String pPayload) {
        if (pHttpCode == 429) {
            ...
            try {
                JsonObject jsonobject = LenientJsonParser.parse(pPayload).getAsJsonObject();
    ...
    public record CustomError(int httpCode, @Nullable Component payload) implements RealmsError {
        ...
        public static RealmsError.CustomError htmlPayload(int pHttpCode, String pPayload) {
            int i = pPayload.indexOf("<body>");
            int j = pPayload.indexOf("</body>");
            if (i >= 0 && j > i) {
                return new RealmsError.CustomError(pHttpCode, Component.literal(pPayload.substring(i + "<body>".length(), j).trim()));
            } else {
                LOGGER.error("Got an error with an unreadable html body {}", pPayload);
                return new RealmsError.CustomError(pHttpCode, null);
            }
        }
