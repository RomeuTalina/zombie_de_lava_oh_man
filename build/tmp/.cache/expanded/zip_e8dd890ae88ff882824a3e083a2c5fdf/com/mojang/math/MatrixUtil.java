...
public class MatrixUtil {
    ...
    private static GivensParameters qrGivensQuat(float pInput1, float pInput2) {
        float f = (float)java.lang.Math.hypot(pInput1, pInput2);
    ...
    private static boolean checkPropertyRaw(Matrix4fc pMatrix, int pProperty) {
        return (pMatrix.properties() & pProperty) != 0;
    }

    public static boolean checkProperty(Matrix4fc pMatrix, int pProperty) {
        if (checkPropertyRaw(pMatrix, pProperty)) {
            return true;
        } else if (pMatrix instanceof Matrix4f matrix4f) {
            matrix4f.determineProperties();
            return checkPropertyRaw(pMatrix, pProperty);
        } else {
            return false;
        }
    }
