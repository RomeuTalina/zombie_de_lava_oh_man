...
public enum OctahedralGroup implements StringRepresentable {
    ...
    private OctahedralGroup(final String pName, final SymmetricGroup3 pPermutation, final boolean pInvertX, final boolean pInvertY, final boolean pInvertZ) {
        ...
        Matrix3f matrix3f = new Matrix3f().scaling(pInvertX ? -1.0F : 1.0F, pInvertY ? -1.0F : 1.0F, pInvertZ ? -1.0F : 1.0F);
        matrix3f.mul(pPermutation.transformation());
    ...
    public Direction.Axis permute(Direction.Axis pAxis) {
        return AXES[this.permutation.permutation(pAxis.ordinal())];
    }
    ...
    public static OctahedralGroup fromXYAngles(Quadrant pX, Quadrant pY) {
        return XY_TABLE[pX.ordinal()][pY.ordinal()];
    }
}