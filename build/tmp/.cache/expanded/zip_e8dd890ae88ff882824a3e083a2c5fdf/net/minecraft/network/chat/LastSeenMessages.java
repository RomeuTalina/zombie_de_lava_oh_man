...
public record LastSeenMessages(List<MessageSignature> entries) {
    ...
    public record Update(int offset, BitSet acknowledged, byte checksum) {
        ...
        public Update(FriendlyByteBuf pBuffer) {
            this(pBuffer.readVarInt(), pBuffer.readFixedBitSet(20), pBuffer.readByte());
        }

        public void write(FriendlyByteBuf pBuffer) {
            pBuffer.writeVarInt(this.offset);
            pBuffer.writeFixedBitSet(this.acknowledged, 20);
            pBuffer.writeByte(this.checksum);
        }

        public boolean verifyChecksum(LastSeenMessages pMessages) {
            return this.checksum == 0 || this.checksum == pMessages.computeChecksum();
        }
    }
}