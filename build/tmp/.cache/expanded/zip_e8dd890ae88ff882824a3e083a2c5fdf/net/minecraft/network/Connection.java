...
public class Connection extends SimpleChannelInboundHandler<Packet<?>> {
    ...
    private <S extends ServerboundPacketListener, C extends ClientboundPacketListener> void initiateServerboundConnection(
        ...
        if (pServerboundProtocol.id() != pClientboundProtocol.id()) {
            ...
            this.runOnceConnected(p_405096_ -> {
                ...
                p_405096_.sendPacket(new ClientIntentionPacket(SharedConstants.getCurrentVersion().protocolVersion(), net.minecraftforge.network.NetworkContext.enhanceHostName(pHostName), pPort, pIntention), null, true);
    ...
    public void send(Packet<?> pPacket, @Nullable ChannelFutureListener pSendListener) {
        this.send(pPacket, pSendListener, true);
    }

    public void send(Packet<?> pPacket, @Nullable ChannelFutureListener pSendListener, boolean pFlush) {
        if (this.isConnected()) {
            this.flushQueue();
            this.sendPacket(pPacket, pSendListener, pFlush);
        } else {
            this.pendingActions.add(p_405086_ -> p_405086_.sendPacket(pPacket, pSendListener, pFlush));
        }
    }
    ...
    private void sendPacket(Packet<?> pPacket, @Nullable ChannelFutureListener pSendListener, boolean pFlush) {
        this.sentPackets++;
    ...
    private void doSendPacket(Packet<?> pPacket, @Nullable ChannelFutureListener pSendListener, boolean pFlush) {
        if (pSendListener != null) {
            ChannelFuture channelfuture = pFlush ? this.channel.writeAndFlush(pPacket) : this.channel.write(pPacket);
            channelfuture.addListener(pSendListener);
            channelfuture.addListener(f -> packetLogger.send(pPacket));
        } else if (pFlush) {
            this.channel.writeAndFlush(pPacket, this.channel.voidPromise());
            packetLogger.send(pPacket);
        } else {
            this.channel.write(pPacket, this.channel.voidPromise());
            packetLogger.send(pPacket);
        }
    }
