...
public record HashedPatchMap(Map<DataComponentType<?>, Integer> addedComponents, Set<DataComponentType<?>> removedComponents) {
    ...
    public static HashedPatchMap create(DataComponentPatch pPatch, HashedPatchMap.HashGenerator pHashGenerator) {
        DataComponentPatch.SplitResult datacomponentpatch$splitresult = pPatch.split();
        Map<DataComponentType<?>, Integer> map = new IdentityHashMap<>(datacomponentpatch$splitresult.added().size());
        datacomponentpatch$splitresult.added().forEach(p_391854_ -> map.put(p_391854_.type(), pHashGenerator.apply((TypedDataComponent<?>)p_391854_)));
        return new HashedPatchMap(map, datacomponentpatch$splitresult.removed());
    }

    public boolean matches(DataComponentPatch pPatch, HashedPatchMap.HashGenerator pHashGenerator) {
        DataComponentPatch.SplitResult datacomponentpatch$splitresult = pPatch.split();
        if (!datacomponentpatch$splitresult.removed().equals(this.removedComponents)) {
            ...
            for (TypedDataComponent<?> typeddatacomponent : datacomponentpatch$splitresult.added()) {
                ...
                Integer integer1 = pHashGenerator.apply(typeddatacomponent);
