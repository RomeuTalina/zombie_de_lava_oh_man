...
public class StringTagVisitor implements TagVisitor {
    ...
    private void handleKeyEscape(String pKey) {
        if (!pKey.equalsIgnoreCase("true") && !pKey.equalsIgnoreCase("false") && UNQUOTED_KEY_MATCH.matcher(pKey).matches()) {
            this.builder.append(pKey);
        } else {
            StringTag.quoteAndEscape(pKey, this.builder);
        }
    }
