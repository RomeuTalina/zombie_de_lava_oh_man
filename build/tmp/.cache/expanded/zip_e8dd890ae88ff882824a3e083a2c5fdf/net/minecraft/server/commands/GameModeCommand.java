...
public class GameModeCommand {
    ...
    private static int setMode(CommandContext<CommandSourceStack> pSource, Collection<ServerPlayer> pPlayers, GameType pGameType) {
        ...
        for (ServerPlayer serverplayer : pPlayers) {
            if (setGameMode(pSource.getSource(), serverplayer, pGameType)) {
                i++;
            }
        }
    ...
    public static void setGameMode(ServerPlayer pPlayer, GameType pGameMode) {
        setGameMode(pPlayer.createCommandSourceStack(), pPlayer, pGameMode);
    }

    private static boolean setGameMode(CommandSourceStack pSource, ServerPlayer pPlayer, GameType pGameMode) {
        if (pPlayer.setGameMode(pGameMode)) {
            logGamemodeChange(pSource, pPlayer, pGameMode);
            return true;
        } else {
            return false;
        }
