...
public class SmithingTrimRecipe implements SmithingRecipe {
    ...
    public SmithingTrimRecipe(Ingredient pTemplate, Ingredient pBase, Ingredient pAddition, Holder<TrimPattern> pPattern) {
    ...
    public static ItemStack applyTrim(HolderLookup.Provider pRegistries, ItemStack pBase, ItemStack pAddition, Holder<TrimPattern> pPattern) {
        Optional<Holder<TrimMaterial>> optional = TrimMaterials.getFromIngredient(pRegistries, pAddition);
        if (optional.isPresent()) {
            ArmorTrim armortrim = pBase.get(DataComponents.TRIM);
            ArmorTrim armortrim1 = new ArmorTrim(optional.get(), pPattern);
