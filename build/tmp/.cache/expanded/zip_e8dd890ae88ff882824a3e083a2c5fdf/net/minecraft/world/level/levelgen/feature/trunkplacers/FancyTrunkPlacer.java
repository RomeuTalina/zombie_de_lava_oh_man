...
public class FancyTrunkPlacer extends TrunkPlacer {
    ...
    private boolean makeLimb(
        ...
        if (!pModifyWorld && Objects.equals(pBasePos, pOffsetPos)) {
            ...
            for (int j = 0; j <= i; j++) {
                BlockPos blockpos1 = pBasePos.offset(Mth.floor(0.5F + j * f), Mth.floor(0.5F + j * f1), Mth.floor(0.5F + j * f2));
                if (pModifyWorld) {
                    this.placeLog(
                        pLevel,
                        pBlockSetter,
    ...
    private boolean trimBranches(int pMaxHeight, int pCurrentHeight) {
        return pCurrentHeight >= pMaxHeight * 0.2;
    }
    ...
    private static float treeShape(int pHeight, int pCurrentY) {
        if (pCurrentY < pHeight * 0.3F) {
            return -1.0F;
        } else {
            float f = pHeight / 2.0F;
            float f1 = f - pCurrentY;
