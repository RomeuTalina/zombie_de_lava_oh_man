...
public record Fireworks(int flightDuration, List<FireworkExplosion> explosions) implements TooltipProvider {
    ...
    private static void addExplosionTooltip(Consumer<Component> pTooltipAdder, FireworkExplosion pExplosion, int pCount) {
        Component component = pExplosion.shape().getName();
        if (pCount == 1) {
            pTooltipAdder.accept(Component.translatable("item.minecraft.firework_rocket.single_star", component).withStyle(ChatFormatting.GRAY));
        } else {
            pTooltipAdder.accept(Component.translatable("item.minecraft.firework_rocket.multiple_stars", pCount, component).withStyle(ChatFormatting.GRAY));
        }

        pExplosion.addAdditionalTooltip(p_329354_ -> pTooltipAdder.accept(Component.literal("  ").append(p_329354_)));
    }
}