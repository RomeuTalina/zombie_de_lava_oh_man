...
public class Node {
    ...
    public float distanceTo(Node pPoint) {
        float f = pPoint.x - this.x;
        float f1 = pPoint.y - this.y;
        float f2 = pPoint.z - this.z;
        return Mth.sqrt(f * f + f1 * f1 + f2 * f2);
    }

    public float distanceToXZ(Node pPoint) {
        float f = pPoint.x - this.x;
        float f1 = pPoint.z - this.z;
        return Mth.sqrt(f * f + f1 * f1);
    }

    public float distanceTo(BlockPos pPos) {
        float f = pPos.getX() - this.x;
        float f1 = pPos.getY() - this.y;
        float f2 = pPos.getZ() - this.z;
        return Mth.sqrt(f * f + f1 * f1 + f2 * f2);
    }
    ...
    public float distanceManhattan(Node pPoint) {
        float f = Math.abs(pPoint.x - this.x);
        float f1 = Math.abs(pPoint.y - this.y);
        float f2 = Math.abs(pPoint.z - this.z);
        return f + f1 + f2;
    }

    public float distanceManhattan(BlockPos pPos) {
        float f = Math.abs(pPos.getX() - this.x);
        float f1 = Math.abs(pPos.getY() - this.y);
        float f2 = Math.abs(pPos.getZ() - this.z);
        return f + f1 + f2;
    }
