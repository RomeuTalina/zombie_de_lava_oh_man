...
public class ScoreboardSaveData extends SavedData {
    ...
    public void loadFrom(ScoreboardSaveData.Packed pPacked) {
        pPacked.objectives().forEach(this.scoreboard::loadObjective);
        pPacked.scores().forEach(this.scoreboard::loadPlayerScore);
        pPacked.displaySlots().forEach((p_391149_, p_391150_) -> {
            Objective objective = this.scoreboard.getObjective(p_391150_);
            this.scoreboard.setDisplayObjective(p_391149_, objective);
        });
        pPacked.teams().forEach(this.scoreboard::loadPlayerTeam);
    }
