...
public class PiglinAi {
    ...
    protected static void wasHurtBy(ServerLevel pLevel, Piglin pPiglin, LivingEntity pEntity) {
        if (!(pEntity instanceof Piglin)) {
            ...
            getAvoidTarget(pPiglin).ifPresent(p_405523_ -> {
                if (p_405523_.getType() != pEntity.getType()) {
    ...
    protected static void broadcastAngerTarget(ServerLevel pLevel, AbstractPiglin pPiglin, LivingEntity pAngerTarget) {
        getAdultPiglins(pPiglin).forEach(p_405528_ -> {
            if (pAngerTarget.getType() != EntityType.HOGLIN || p_405528_.canHunt() && ((Hoglin)pAngerTarget).canBeHunted()) {
                setAngerTargetIfCloserThanCurrent(pLevel, p_405528_, pAngerTarget);
            }
        });
    }
