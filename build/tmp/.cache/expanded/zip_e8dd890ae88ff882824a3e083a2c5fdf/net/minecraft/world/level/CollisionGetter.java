...
public interface CollisionGetter extends BlockGetter {
    ...
    default Iterable<VoxelShape> getPreMoveCollisions(@Nullable Entity pEntity, AABB pCollisionBox, Vec3 pPos) {
        List<VoxelShape> list = this.getEntityCollisions(pEntity, pCollisionBox);
        Iterable<VoxelShape> iterable = this.getBlockCollisionsFromContext(CollisionContext.withPosition(pEntity, pPos.y), pCollisionBox);
        return list.isEmpty() ? iterable : Iterables.concat(list, iterable);
    }

    default Iterable<VoxelShape> getBlockCollisions(@Nullable Entity pEntity, AABB pCollisionBox) {
        return this.getBlockCollisionsFromContext(pEntity == null ? CollisionContext.empty() : CollisionContext.of(pEntity), pCollisionBox);
    }

    default Iterable<VoxelShape> getBlockAndLiquidCollisions(@Nullable Entity pEntity, AABB pCollisionBox) {
        return this.getBlockCollisionsFromContext(pEntity == null ? CollisionContext.empty() : CollisionContext.of(pEntity, true), pCollisionBox);
    }

    private Iterable<VoxelShape> getBlockCollisionsFromContext(CollisionContext pContext, AABB pCollisionBox) {
        return () -> new BlockCollisions<>(this, pContext, pCollisionBox, false, (p_286215_, p_286216_) -> p_286216_);
    }
