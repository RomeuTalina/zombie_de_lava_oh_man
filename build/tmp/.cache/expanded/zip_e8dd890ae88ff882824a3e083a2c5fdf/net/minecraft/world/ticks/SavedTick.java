...
public record SavedTick<T>(T type, BlockPos pos, int delay, TickPriority priority) {
    ...
    public static <T> Codec<SavedTick<T>> codec(Codec<T> pCodec) {
        ...
        return RecordCodecBuilder.create(
            p_391162_ -> p_391162_.group(
                    pCodec.fieldOf("i").forGetter(SavedTick::type),
    ...
    public static <T> List<SavedTick<T>> filterTickListForChunk(List<SavedTick<T>> pTickList, ChunkPos pChunkPos) {
        long i = pChunkPos.toLong();
        return pTickList.stream().filter(p_391159_ -> ChunkPos.asLong(p_391159_.pos()) == i).toList();
    }

    public ScheduledTick<T> unpack(long pGameTime, long pSubTickOrder) {
        return new ScheduledTick<>(this.type, this.pos, pGameTime + this.delay, this.priority, pSubTickOrder);
    }
