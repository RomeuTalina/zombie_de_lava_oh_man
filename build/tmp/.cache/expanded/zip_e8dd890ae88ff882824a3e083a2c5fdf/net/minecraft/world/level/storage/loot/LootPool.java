...
public class LootPool {
    ...
    public void validate(ValidationContext pContext) {
        for (int i = 0; i < this.conditions.size(); i++) {
            this.conditions.get(i).validate(pContext.forChild(new ProblemReporter.IndexedFieldPathElement("conditions", i)));
        }

        for (int j = 0; j < this.functions.size(); j++) {
            this.functions.get(j).validate(pContext.forChild(new ProblemReporter.IndexedFieldPathElement("functions", j)));
        }

        for (int k = 0; k < this.entries.size(); k++) {
            this.entries.get(k).validate(pContext.forChild(new ProblemReporter.IndexedFieldPathElement("entries", k)));
        }

        this.rolls.validate(pContext.forChild(new ProblemReporter.FieldPathElement("rolls")));
        this.bonusRolls.validate(pContext.forChild(new ProblemReporter.FieldPathElement("bonus_rolls")));
    }
