...
public class CreakingAi {
    ...
    static void initFightActivity(Creaking pCreaking, Brain<Creaking> pBrain) {
        pBrain.addActivityWithConditions(
            ...
            ImmutableList.of(
                ...
                StopAttackingIfTargetInvalid.create((p_390695_, p_390696_) -> !isAttackTargetStillReachable(pCreaking, p_390696_))
            ),
            ImmutableSet.of(Pair.of(MemoryModuleType.ATTACK_TARGET, MemoryStatus.VALUE_PRESENT))
        );
    }

    private static boolean isAttackTargetStillReachable(Creaking pCreaking, LivingEntity pTarget) {
        Optional<List<Player>> optional = pCreaking.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_ATTACKABLE_PLAYERS);
        return optional.<Boolean>map(p_390693_ -> pTarget instanceof Player player && p_390693_.contains(player)).orElse(false);
    }
    ...
    public static Brain<Creaking> makeBrain(Creaking pCreaking, Brain<Creaking> pBrain) {
        initCoreActivity(pBrain);
        initIdleActivity(pBrain);
        initFightActivity(pCreaking, pBrain);
