...
public class AxeItem extends Item {
    ...
    private Optional<BlockState> evaluateNewBlockState(Level pLevel, BlockPos pPos, @Nullable Player pPlayer, BlockState pState) {
        return this.evaluateNewBlockState(pLevel, pPos, pPlayer, pState, null);
    }

    private Optional<BlockState> evaluateNewBlockState(Level pLevel, BlockPos pPos, @Nullable Player pPlayer, BlockState pState, @Nullable UseOnContext ctx) {
        var strip = ctx == null ? null : pState.getToolModifiedState(ctx, net.minecraftforge.common.ToolActions.AXE_STRIP, false);
        Optional<BlockState> optional = strip != null ? Optional.of(strip) : this.getStripped(pState);
        if (optional.isPresent()) {
            pLevel.playSound(pPlayer, pPos, SoundEvents.AXE_STRIP, SoundSource.BLOCKS, 1.0F, 1.0F);
            return optional;
        } else {
            var scrape = ctx == null ? null : pState.getToolModifiedState(ctx, net.minecraftforge.common.ToolActions.AXE_STRIP, false);
            Optional<BlockState> optional1 = scrape != null ? Optional.of(scrape) : WeatheringCopper.getPrevious(pState);
            if (optional1.isPresent()) {
                ...
                var waxOff = ctx == null ? null : pState.getToolModifiedState(ctx, net.minecraftforge.common.ToolActions.AXE_WAX_OFF, false);
                Optional<BlockState> optional2 = waxOff != null ? Optional.of(waxOff) : Optional.ofNullable(HoneycombItem.WAX_OFF_BY_BLOCK.get().get(pState.getBlock()))
