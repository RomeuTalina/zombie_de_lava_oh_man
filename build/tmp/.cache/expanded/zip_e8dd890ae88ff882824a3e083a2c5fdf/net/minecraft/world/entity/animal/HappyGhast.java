...
public class HappyGhast extends Animal {
    ...
    private void setServerStillTimeout(int pServerStillTimeout) {
        if (this.serverStillTimeout <= 0 && pServerStillTimeout > 0 && this.level() instanceof ServerLevel serverlevel) {
            this.syncPacketPositionCodec(this.getX(), this.getY(), this.getZ());
            serverlevel.getChunkSource().chunkMap.broadcast(this, ClientboundEntityPositionSyncPacket.of(this));
        }

        this.serverStillTimeout = pServerStillTimeout;
        this.syncStayStillFlag();
    }

    private PathNavigation createBabyNavigation(Level pLevel) {
        return new HappyGhast.BabyFlyingPathNavigation(this, pLevel);
    }
    ...
    private void doPlayerRide(Player pPlayer) {
    ...
    private void setLeashHolder(boolean pLeashHolder) {
        this.entityData.set(IS_LEASH_HOLDER, pLeashHolder);
    }
    ...
    static class BabyFlyingPathNavigation extends FlyingPathNavigation {
        public BabyFlyingPathNavigation(HappyGhast pGhast, Level pLevel) {
            super(pGhast, pLevel);
    ...
    class HappyGhastLookControl extends LookControl {
        ...
        public static float wrapDegrees90(float pDegrees) {
            float f = pDegrees % 90.0F;
            if (f >= 45.0F) {
                f -= 90.0F;
            }
