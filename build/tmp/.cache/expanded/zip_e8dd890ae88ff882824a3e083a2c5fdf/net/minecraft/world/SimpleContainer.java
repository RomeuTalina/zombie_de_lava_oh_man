...
public class SimpleContainer implements Container, StackedContentsCompatible {
    ...
    public void fromItemList(ValueInput.TypedInputList<ItemStack> pInput) {
        this.clearContent();

        for (ItemStack itemstack : pInput) {
            this.addItem(itemstack);
        }
    }

    public void storeAsItemList(ValueOutput.TypedOutputList<ItemStack> pOutput) {
        for (int i = 0; i < this.getContainerSize(); i++) {
            ItemStack itemstack = this.getItem(i);
            if (!itemstack.isEmpty()) {
                pOutput.add(itemstack);
            }
        }
    }
