...
public class SurfaceSystem {
        ...
        BlockColumn blockcolumn = new BlockColumn() {
            ...
            public void setBlock(int p_190008_, BlockState p_190009_) {
                ...
                if (levelheightaccessor.isInsideBuildHeight(p_190008_)) {
                    pChunk.setBlockState(blockpos$mutableblockpos.setY(p_190008_), p_190009_);
    ...
    private void erodedBadlandsExtension(BlockColumn pBlockColumn, int pX, int pZ, int pHeight, LevelHeightAccessor pLevel) {
        double d0 = 0.2;
        double d1 = Math.min(
            Math.abs(this.badlandsSurfaceNoise.getValue(pX, 0.0, pZ) * 8.25), this.badlandsPillarNoise.getValue(pX * 0.2, 0.0, pZ * 0.2) * 15.0
        );
        if (!(d1 <= 0.0)) {
            double d2 = 0.75;
            double d3 = 1.5;
            double d4 = Math.abs(this.badlandsPillarRoofNoise.getValue(pX * 0.75, 0.0, pZ * 0.75) * 1.5);
    ...
    private void frozenOceanExtension(
        ...
        double d0 = 1.28;
        double d1 = Math.min(
            Math.abs(this.icebergSurfaceNoise.getValue(pX, 0.0, pZ) * 8.25), this.icebergPillarNoise.getValue(pX * 1.28, 0.0, pZ * 1.28) * 15.0
        );
        if (!(d1 <= 1.8)) {
            double d3 = 1.17;
            double d4 = 1.5;
            double d5 = Math.abs(this.icebergPillarRoofNoise.getValue(pX * 1.17, 0.0, pZ * 1.17) * 1.5);
    ...
    protected BlockState getBand(int pX, int pY, int pZ) {
        int i = (int)Math.round(this.clayBandsOffsetNoise.getValue(pX, 0.0, pZ) * 4.0);
