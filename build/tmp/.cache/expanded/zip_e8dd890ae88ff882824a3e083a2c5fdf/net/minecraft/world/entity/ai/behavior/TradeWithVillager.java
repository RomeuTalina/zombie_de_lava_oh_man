...
public class TradeWithVillager extends Behavior<Villager> {
    ...
    protected void tick(ServerLevel pLevel, Villager pOwner, long pGameTime) {
        ...
        if (!(pOwner.distanceToSqr(villager) > 5.0)) {
            ...
            boolean flag = pOwner.getVillagerData().profession().is(VillagerProfession.FARMER);
            if (pOwner.hasExcessFood() && (flag || villager.wantsMoreFood())) {
                throwHalfStack(pOwner, Villager.FOOD_POINTS.keySet(), villager);
            }

            if (flag && pOwner.getInventory().countItem(Items.WHEAT) > Items.WHEAT.getDefaultMaxStackSize() / 2) {
    ...
    private static Set<Item> figureOutWhatIAmWillingToTrade(Villager pVillager, Villager pOther) {
        ImmutableSet<Item> immutableset = pOther.getVillagerData().profession().value().requestedItems();
        ImmutableSet<Item> immutableset1 = pVillager.getVillagerData().profession().value().requestedItems();
