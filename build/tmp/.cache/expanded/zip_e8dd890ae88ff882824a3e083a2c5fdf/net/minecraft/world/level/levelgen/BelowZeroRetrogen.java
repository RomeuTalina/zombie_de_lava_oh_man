...
public final class BelowZeroRetrogen {
    ...
    public static void replaceOldBedrock(ProtoChunk pChunk) {
        ...
        BlockPos.betweenClosed(0, 0, 0, 15, 4, 15).forEach(p_391033_ -> {
            if (pChunk.getBlockState(p_391033_).is(Blocks.BEDROCK)) {
                pChunk.setBlockState(p_391033_, Blocks.DEEPSLATE.defaultBlockState());
            }
        });
    }
        ...
        for (int k = 0; k < 16; k++) {
            for (int l = 0; l < 16; l++) {
                if (this.hasBedrockHole(k, l)) {
                    BlockPos.betweenClosed(k, i, l, k, j, l).forEach(p_391035_ -> pChunk.setBlockState(p_391035_, Blocks.AIR.defaultBlockState()));
                }
            }
        }
    }
