...
public record WrittenBookContent(Filterable<String> title, String author, int generation, List<Filterable<Component>> pages, boolean resolved)
    ...
    public static boolean resolveForItem(ItemStack pStack, CommandSourceStack pSource, @Nullable Player pPlayer) {
        ...
        if (writtenbookcontent != null && !writtenbookcontent.resolved()) {
            WrittenBookContent writtenbookcontent1 = writtenbookcontent.resolve(pSource, pPlayer);
            if (writtenbookcontent1 != null) {
                pStack.set(DataComponents.WRITTEN_BOOK_CONTENT, writtenbookcontent1);
                return true;
            }

            pStack.set(DataComponents.WRITTEN_BOOK_CONTENT, writtenbookcontent.markResolved());
        }
    ...
    private static boolean isPageTooLarge(Component pPage, HolderLookup.Provider pRegistryAccess) {
        DataResult<JsonElement> dataresult = ComponentSerialization.CODEC.encodeStart(pRegistryAccess.createSerializationContext(JsonOps.INSTANCE), pPage);
