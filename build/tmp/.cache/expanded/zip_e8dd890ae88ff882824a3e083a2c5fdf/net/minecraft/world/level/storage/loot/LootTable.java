...
public class LootTable {
    ...
    public void validate(ValidationContext pValidator) {
        for (int i = 0; i < this.pools.size(); i++) {
            this.pools.get(i).validate(pValidator.forChild(new ProblemReporter.IndexedFieldPathElement("pools", i)));
        }

        for (int j = 0; j < this.functions.size(); j++) {
            this.functions.get(j).validate(pValidator.forChild(new ProblemReporter.IndexedFieldPathElement("functions", j)));
        }
    }
