...
public class Scoreboard {
    ...
    protected void loadPlayerScore(Scoreboard.PackedScore pScore) {
        Objective objective = this.getObjective(pScore.objective);
        if (objective == null) {
            LOGGER.error("Unknown objective {} for name {}, ignoring", pScore.objective, pScore.owner);
        } else {
            this.getOrCreatePlayerInfo(pScore.owner).setScore(objective, pScore.score);
        }
    }

    protected void loadPlayerTeam(PlayerTeam.Packed pPacked) {
        PlayerTeam playerteam = this.addPlayerTeam(pPacked.name());
        pPacked.displayName().ifPresent(playerteam::setDisplayName);
        pPacked.color().ifPresent(playerteam::setColor);
        playerteam.setAllowFriendlyFire(pPacked.allowFriendlyFire());
        playerteam.setSeeFriendlyInvisibles(pPacked.seeFriendlyInvisibles());
        playerteam.setPlayerPrefix(pPacked.memberNamePrefix());
        playerteam.setPlayerSuffix(pPacked.memberNameSuffix());
        playerteam.setNameTagVisibility(pPacked.nameTagVisibility());
        playerteam.setDeathMessageVisibility(pPacked.deathMessageVisibility());
        playerteam.setCollisionRule(pPacked.collisionRule());

        for (String s : pPacked.players()) {
            this.addPlayerToTeam(s, playerteam);
        }
    }

    protected void loadObjective(Objective.Packed pPacked) {
        this.addObjective(
            pPacked.name(),
            pPacked.criteria(),
            pPacked.displayName(),
            pPacked.renderType(),
            pPacked.displayAutoUpdate(),
            pPacked.numberFormat().orElse(null)
        );
    }
