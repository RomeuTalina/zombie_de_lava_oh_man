    ...
    public boolean growTree(ServerLevel pLevel, ChunkGenerator pChunkGenerator, BlockPos pPos, BlockState pState, RandomSource pRandom) {
                ...
                for (int i = 0; i >= -1; i--) {
                    for (int j = 0; j >= -1; j--) {
                        if (isTwoByTwoSapling(pState, pLevel, pPos, i, j)) {
                            ...
                            pLevel.setBlock(pPos.offset(i, 0, j), blockstate, 260);
                            pLevel.setBlock(pPos.offset(i + 1, 0, j), blockstate, 260);
                            pLevel.setBlock(pPos.offset(i, 0, j + 1), blockstate, 260);
                            pLevel.setBlock(pPos.offset(i + 1, 0, j + 1), blockstate, 260);
                            if (configuredfeature.place(pLevel, pChunkGenerator, pRandom, pPos.offset(i, 0, j))) {
                                return true;
                            }

                            pLevel.setBlock(pPos.offset(i, 0, j), pState, 260);
                            pLevel.setBlock(pPos.offset(i + 1, 0, j), pState, 260);
                            pLevel.setBlock(pPos.offset(i, 0, j + 1), pState, 260);
                            pLevel.setBlock(pPos.offset(i + 1, 0, j + 1), pState, 260);
                            return false;
                        }
                    }
                }
        ...
        if (resourcekey1 == null) {
            ...
            if (holder1 == null) {
                ...
                pLevel.setBlock(pPos, blockstate1, 260);
                if (configuredfeature1.place(pLevel, pChunkGenerator, pRandom, pPos)) {
                    ...
                    pLevel.setBlock(pPos, pState, 260);
                    return false;
                }
            }
        }
