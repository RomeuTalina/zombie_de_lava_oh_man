...
public record TooltipDisplay(boolean hideTooltip, SequencedSet<DataComponentType<?>> hiddenComponents) {
    ...
    public TooltipDisplay withHidden(DataComponentType<?> pComponent, boolean pIsHidden) {
        if (this.hiddenComponents.contains(pComponent) == pIsHidden) {
            return this;
        } else {
            SequencedSet<DataComponentType<?>> sequencedset = new ReferenceLinkedOpenHashSet<>(this.hiddenComponents);
            if (pIsHidden) {
                sequencedset.add(pComponent);
            } else {
                sequencedset.remove(pComponent);
            }
    ...
    public boolean shows(DataComponentType<?> pComponent) {
        return !this.hideTooltip && !this.hiddenComponents.contains(pComponent);
    }
}