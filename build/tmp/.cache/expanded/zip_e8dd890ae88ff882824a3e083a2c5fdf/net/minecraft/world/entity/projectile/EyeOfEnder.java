...
public class EyeOfEnder extends Entity implements ItemSupplier {
    ...
    public void signalTo(Vec3 pPos) {
        Vec3 vec3 = pPos.subtract(this.position());
    ...
    private void spawnParticles(Vec3 pPos, Vec3 pDeltaMovement) {
        if (this.isInWater()) {
            for (int i = 0; i < 4; i++) {
                this.level()
                    .addParticle(
            ...
            this.level()
                .addParticle(
                    ...
                    pPos.x + this.random.nextDouble() * 0.6 - 0.3,
                    pPos.y - 0.5,
                    pPos.z + this.random.nextDouble() * 0.6 - 0.3,
                    pDeltaMovement.x,
                    pDeltaMovement.y,
                    pDeltaMovement.z
                );
    ...
    private static Vec3 updateDeltaMovement(Vec3 pDeltaMovement, Vec3 pPos, Vec3 pTarget) {
        Vec3 vec3 = new Vec3(pTarget.x - pPos.x, 0.0, pTarget.z - pPos.z);
        double d0 = vec3.length();
        double d1 = Mth.lerp(0.0025, pDeltaMovement.horizontalDistance(), d0);
        ...
        double d3 = pPos.y - pDeltaMovement.y < pTarget.y ? 1.0 : -1.0;
