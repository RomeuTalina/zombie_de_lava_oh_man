...
public record ArmorTrim(Holder<TrimMaterial> material, Holder<TrimPattern> pattern) implements TooltipProvider {
    ...
    public ResourceLocation layerAssetId(String pPrefix, ResourceKey<EquipmentAsset> pEquipmentAssetId) {
        MaterialAssetGroup.AssetInfo materialassetgroup$assetinfo = this.material().value().assets().assetId(pEquipmentAssetId);
        return this.pattern().value().assetId().withPath(p_390861_ -> pPrefix + "/" + p_390861_ + "_" + materialassetgroup$assetinfo.suffix());
    }
}