...
public class AABB {
    ...
    public boolean intersects(BlockPos pPos) {
        return this.intersects(
            pPos.getX(),
            pPos.getY(),
            pPos.getZ(),
    ...
    public double distanceToSqr(AABB pBox) {
        double d0 = Math.max(Math.max(this.minX - pBox.maxX, pBox.minX - this.maxX), 0.0);
        double d1 = Math.max(Math.max(this.minY - pBox.maxY, pBox.minY - this.maxY), 0.0);
        double d2 = Math.max(Math.max(this.minZ - pBox.maxZ, pBox.minZ - this.maxZ), 0.0);
        return Mth.lengthSquared(d0, d1, d2);
    }
