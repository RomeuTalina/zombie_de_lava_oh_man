...
public final class ProjectileUtil {
    ...
    public static HitResult getHitResultOnMoveVector(Entity pProjectile, Predicate<Entity> pFilter) {
        ...
        return getHitResult(vec31, pProjectile, pFilter, vec3, level, computeMargin(pProjectile), ClipContext.Block.COLLIDER);
    }
    ...
    public static HitResult getHitResultOnMoveVector(Entity pProjectile, Predicate<Entity> pFilter, ClipContext.Block pClipContext) {
        ...
        Level level = pProjectile.level();
        Vec3 vec31 = pProjectile.position();
        return getHitResult(vec31, pProjectile, pFilter, vec3, level, computeMargin(pProjectile), pClipContext);
    }
    ...
    @Nullable
    public static EntityHitResult getEntityHitResult(Level pLevel, Projectile pProjectile, Vec3 pStartVec, Vec3 pEndVec, AABB pBoundingBox, Predicate<Entity> pFilter) {
        return getEntityHitResult(pLevel, pProjectile, pStartVec, pEndVec, pBoundingBox, pFilter, computeMargin(pProjectile));
    }

    public static float computeMargin(Entity pEntity) {
        return Math.max(0.0F, Math.min(0.3F, (pEntity.tickCount - 2) / 20.0F));
    }
