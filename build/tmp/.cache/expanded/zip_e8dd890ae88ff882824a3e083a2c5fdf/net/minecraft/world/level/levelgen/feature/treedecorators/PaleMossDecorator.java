...
public class PaleMossDecorator extends TreeDecorator {
    ...
    private static void addMossHanger(BlockPos pPos, TreeDecorator.Context pContext) {
        while (pContext.isAir(pPos.below()) && !(pContext.random().nextFloat() < 0.5)) {
            pContext.setBlock(pPos, Blocks.PALE_HANGING_MOSS.defaultBlockState().setValue(HangingMossBlock.TIP, false));
            pPos = pPos.below();
        }

        pContext.setBlock(pPos, Blocks.PALE_HANGING_MOSS.defaultBlockState().setValue(HangingMossBlock.TIP, true));
    }
}