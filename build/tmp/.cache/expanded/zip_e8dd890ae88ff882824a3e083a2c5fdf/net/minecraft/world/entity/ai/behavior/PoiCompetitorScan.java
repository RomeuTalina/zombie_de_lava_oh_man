...
public class PoiCompetitorScan {
    ...
    private static boolean competesForSameJobsite(GlobalPos pJobSitePos, Holder<PoiType> pPoiType, Villager pVillager) {
        Optional<GlobalPos> optional = pVillager.getBrain().getMemory(MemoryModuleType.JOB_SITE);
        return optional.isPresent() && pJobSitePos.equals(optional.get()) && hasMatchingProfession(pPoiType, pVillager.getVillagerData().profession());
    }

    private static boolean hasMatchingProfession(Holder<PoiType> pPoiType, Holder<VillagerProfession> pProfession) {
        return pProfession.value().heldJobSite().test(pPoiType);
    }
}