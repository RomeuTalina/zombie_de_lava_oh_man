...
public class TrialSpawnerStateData {
    ...
    public void apply(TrialSpawnerStateData.Packed pData) {
        this.detectedPlayers.clear();
        this.detectedPlayers.addAll(pData.detectedPlayers);
        this.currentMobs.clear();
        this.currentMobs.addAll(pData.currentMobs);
        this.cooldownEndsAt = pData.cooldownEndsAt;
        this.nextMobSpawnsAt = pData.nextMobSpawnsAt;
        this.totalMobsSpawned = pData.totalMobsSpawned;
        this.nextSpawnData = pData.nextSpawnData;
        this.ejectingLootTable = pData.ejectingLootTable;
    }
    ...
    public boolean hasMobToSpawn(TrialSpawner pSpawner, RandomSource pRandom) {
        boolean flag = this.getOrCreateNextSpawnData(pSpawner, pRandom).getEntityToSpawn().getString("id").isPresent();
        return flag || !pSpawner.activeConfig().spawnPotentialsDefinition().isEmpty();
    }
        ...
        if (!flag) {
            if (!pSpawner.getState().equals(TrialSpawnerState.COOLDOWN) || !pSpawner.isOminous()) {
                ...
                if (!pSpawner.getState().equals(TrialSpawnerState.COOLDOWN) || flag1) {
                    boolean flag2 = pSpawner.getStateData().detectedPlayers.isEmpty();
    ...
    public void resetAfterBecomingOminous(TrialSpawner pSpawner, ServerLevel pLevel) {
        this.currentMobs.stream().map(pLevel::getEntity).forEach(p_410138_ -> {
            if (p_410138_ != null) {
                pLevel.levelEvent(3012, p_410138_.blockPosition(), TrialSpawner.FlameParticle.NORMAL.encode());
        ...
        if (!pSpawner.ominousConfig().spawnPotentialsDefinition().isEmpty()) {
            this.nextSpawnData = Optional.empty();
        }

        this.totalMobsSpawned = 0;
        this.currentMobs.clear();
        this.nextMobSpawnsAt = pLevel.getGameTime() + pSpawner.ominousConfig().ticksBetweenSpawn();
        pSpawner.markUpdated();
        this.cooldownEndsAt = pLevel.getGameTime() + pSpawner.ominousConfig().ticksBetweenItemSpawners();
    }
    ...
    protected SpawnData getOrCreateNextSpawnData(TrialSpawner pSpawner, RandomSource pRandom) {
        if (this.nextSpawnData.isPresent()) {
            ...
            WeightedList<SpawnData> weightedlist = pSpawner.activeConfig().spawnPotentialsDefinition();
