...
public class EndPodiumFeature extends Feature<NoneFeatureConfiguration> {
    ...
    private void dropPreviousAndSetBlock(WorldGenLevel pLevel, BlockPos pPos, Block pBlock) {
        if (!pLevel.getBlockState(pPos).is(pBlock)) {
            pLevel.destroyBlock(pPos, true, null);
            this.setBlock(pLevel, pPos, pBlock.defaultBlockState());
        }
    }
}