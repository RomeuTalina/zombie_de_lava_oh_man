...
public class ShulkerBullet extends Projectile {
    ...
    public ShulkerBullet(Level pLevel, LivingEntity pShooter, Entity pFinalTarget, Direction.Axis pAxis) {
        ...
        this.finalTarget = new EntityReference<>(pFinalTarget);
        this.currentMoveDirection = Direction.UP;
        this.selectNextMoveDirection(pAxis, pFinalTarget);
    }
    ...
    private void selectNextMoveDirection(@Nullable Direction.Axis pAxis, @Nullable Entity pTarget) {
        double d0 = 0.5;
        BlockPos blockpos;
        if (pTarget == null) {
            blockpos = this.blockPosition().below();
        } else {
            d0 = pTarget.getBbHeight() * 0.5;
            blockpos = BlockPos.containing(pTarget.getX(), pTarget.getY() + d0, pTarget.getZ());
        }
