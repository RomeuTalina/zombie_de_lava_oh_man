            ...
            if (pLevel.getBlockState(blockpos).isAir() && worldborder.isWithinBounds(blockpos)) {
                if (pLevel.isClientSide) {
                    for (int j = 0; j < 128; j++) {
                        ...
                        double d1 = Mth.lerp(d0, blockpos.getX(), pPos.getX()) + (pLevel.random.nextDouble() - 0.5) + 0.5;
                        double d2 = Mth.lerp(d0, blockpos.getY(), pPos.getY()) + pLevel.random.nextDouble() - 0.5;
                        double d3 = Mth.lerp(d0, blockpos.getZ(), pPos.getZ()) + (pLevel.random.nextDouble() - 0.5) + 0.5;
                        pLevel.addParticle(ParticleTypes.PORTAL, d1, d2, d3, f, f1, f2);
                    }
                } else {
