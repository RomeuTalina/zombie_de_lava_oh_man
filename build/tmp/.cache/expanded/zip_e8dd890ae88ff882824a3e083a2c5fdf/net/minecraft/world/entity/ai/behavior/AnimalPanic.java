...
public class AnimalPanic<E extends PathfinderMob> extends Behavior<E> {
    ...
    public AnimalPanic(float pSpeedMultiplier) {
        this(pSpeedMultiplier, p_341293_ -> DamageTypeTags.PANIC_CAUSES, p_405308_ -> LandRandomPos.getPos(p_405308_, 5, 4));
    }

    public AnimalPanic(float pSpeedMultiplier, int pY) {
        this(
            pSpeedMultiplier,
            p_405304_ -> DamageTypeTags.PANIC_CAUSES,
            p_405307_ -> AirAndWaterRandomPos.getPos(
                p_405307_, 5, 4, pY, p_405307_.getViewVector(0.0F).x, p_405307_.getViewVector(0.0F).z, (float) (Math.PI / 2)
            )
        );
    }

    public AnimalPanic(float pSpeedMultiplier, Function<PathfinderMob, TagKey<DamageType>> pPanicCausingDamageTypes) {
        this(pSpeedMultiplier, pPanicCausingDamageTypes, p_405305_ -> LandRandomPos.getPos(p_405305_, 5, 4));
    }

    public AnimalPanic(float pSpeedMultiplier, Function<PathfinderMob, TagKey<DamageType>> pPanicCausingDamageTypes, Function<E, Vec3> pPositionGetter) {
        ...
        this.positionGetter = pPositionGetter;
    }
    ...
    private Vec3 getPanicPos(E pPathfinder, ServerLevel pLevel) {
        ...
        return this.positionGetter.apply(pPathfinder);
    }
