...
public interface Bucketable {
    ...
    static void saveDefaultDataToBucketTag(Mob pMob, ItemStack pBucket) {
        pBucket.copyFrom(DataComponents.CUSTOM_NAME, pMob);
    ...
    static void loadDefaultDataFromBucketTag(Mob pMob, CompoundTag pTag) {
        pTag.getBoolean("NoAI").ifPresent(pMob::setNoAi);
        pTag.getBoolean("Silent").ifPresent(pMob::setSilent);
        pTag.getBoolean("NoGravity").ifPresent(pMob::setNoGravity);
        pTag.getBoolean("Glowing").ifPresent(pMob::setGlowingTag);
        pTag.getBoolean("Invulnerable").ifPresent(pMob::setInvulnerable);
        pTag.getFloat("Health").ifPresent(pMob::setHealth);
    }
