...
public class GossipContainer {
    ...
    private GossipContainer(List<GossipContainer.GossipEntry> pEntries) {
        pEntries.forEach(p_26162_ -> this.getOrCreate(p_26162_.target).entries.put(p_26162_.type, p_26162_.value));
    }
    ...
    public long getCountForType(GossipType pGossipType, DoublePredicate pGossipPredicate) {
        return this.gossips.values().stream().filter(p_148174_ -> pGossipPredicate.test(p_148174_.entries.getOrDefault(pGossipType, 0) * pGossipType.weight)).count();
    }
    ...
    public void putAll(GossipContainer pOther) {
        pOther.gossips.forEach((p_390636_, p_390637_) -> this.getOrCreate(p_390636_).entries.putAll(p_390637_.entries));
    }
