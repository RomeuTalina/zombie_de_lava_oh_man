...
public class PlayerTabOverlay {
    ...
    protected void renderPingIcon(GuiGraphics pGuiGraphics, int pWidth, int pX, int pY, PlayerInfo pPlayerInfo) {
        ...
        pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, resourcelocation, pX + pWidth - 11, pY, 10, 8);
    }
    ...
    private void renderTablistScore(
        ...
        if (pObjective.getRenderType() == ObjectiveCriteria.RenderType.HEARTS) {
            ...
            pGuiGraphics.drawString(this.minecraft.font, pDisplayEntry.formattedScore, pMaxX - pDisplayEntry.scoreWidth, pY, -1);
        }
    }
        ...
        if (i > 0) {
            ...
            if (k <= 3) {
                ...
                pGuiGraphics.drawString(
                    this.minecraft.font,
                    component1,
                    (pMaxX + pMinX - this.minecraft.font.width(component1)) / 2,
                    pY,
                    ARGB.opaque(j1)
                );
            } else {
                ...
                for (int l = i; l < j; l++) {
                    pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, resourcelocation, pMinX + l * k, pY, 9, 9);
                }

                for (int i1 = 0; i1 < i; i1++) {
                    pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, resourcelocation, pMinX + i1 * k, pY, 9, 9);
                    if (flag) {
                        if (i1 * 2 + 1 < playertaboverlay$healthstate.displayedValue()) {
                            pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, HEART_FULL_BLINKING_SPRITE, pMinX + i1 * k, pY, 9, 9);
                        }

                        if (i1 * 2 + 1 == playertaboverlay$healthstate.displayedValue()) {
                            pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, HEART_HALF_BLINKING_SPRITE, pMinX + i1 * k, pY, 9, 9);
                        }
                    }

                    if (i1 * 2 + 1 < pHealth) {
                        pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, i1 >= 10 ? HEART_ABSORBING_FULL_BLINKING_SPRITE : HEART_FULL_SPRITE, pMinX + i1 * k, pY, 9, 9);
                    }

                    if (i1 * 2 + 1 == pHealth) {
                        pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, i1 >= 10 ? HEART_ABSORBING_HALF_BLINKING_SPRITE : HEART_HALF_SPRITE, pMinX + i1 * k, pY, 9, 9);
                    }
                }
            }
        }
