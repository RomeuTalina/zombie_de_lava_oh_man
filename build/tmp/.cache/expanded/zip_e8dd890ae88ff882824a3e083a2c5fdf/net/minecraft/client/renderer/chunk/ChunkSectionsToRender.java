...
public record ChunkSectionsToRender(EnumMap<ChunkSectionLayer, List<RenderPass.Draw<GpuBufferSlice[]>>> drawsPerLayer, int maxIndicesRequired, GpuBufferSlice[] dynamicTransforms) {
    public void renderGroup(ChunkSectionLayerGroup pGroup) {
        ...
        ChunkSectionLayer[] achunksectionlayer = pGroup.layers();
        Minecraft minecraft = Minecraft.getInstance();
        boolean flag = false;
        RenderTarget rendertarget = pGroup.outputTarget();

        try (RenderPass renderpass = RenderSystem.getDevice()
                .createCommandEncoder()
                .createRenderPass(
                    () -> "Section layers for " + pGroup.label(),
