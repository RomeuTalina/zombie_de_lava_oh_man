...
public abstract class Particle {
    ...
    public Particle setPower(float pMultiplier) {
        this.xd *= pMultiplier;
        this.yd = (this.yd - 0.1F) * pMultiplier + 0.1F;
        this.zd *= pMultiplier;
        return this;
    }
    ...
    protected void setSize(float pWidth, float pHeight) {
        if (pWidth != this.bbWidth || pHeight != this.bbHeight) {
            ...
            double d0 = (aabb.minX + aabb.maxX - pWidth) / 2.0;
            double d1 = (aabb.minZ + aabb.maxZ - pWidth) / 2.0;
    ...
    public void setPos(double pX, double pY, double pZ) {
        ...
        this.setBoundingBox(new AABB(pX - f, pY, pZ - f, pX + f, pY + f1, pZ + f));
    }
    ...
    public record LifetimeAlpha(float startAlpha, float endAlpha, float startAtNormalizedAge, float endAtNormalizedAge) {
        ...
        public float currentAlphaForAge(int pAge, int pLifetime, float pPartialTick) {
            if (Mth.equal(this.startAlpha, this.endAlpha)) {
                ...
                float f = Mth.inverseLerp((pAge + pPartialTick) / pLifetime, this.startAtNormalizedAge, this.endAtNormalizedAge);
