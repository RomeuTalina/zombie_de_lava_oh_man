...
public class ItemInHandRenderer {
    ...
    private void applyEatTransform(PoseStack pPoseStack, float pPartialTick, HumanoidArm pArm, ItemStack pStack, Player pPlayer) {
        float f = pPlayer.getUseItemRemainingTicks() - pPartialTick + 1.0F;
        float f1 = f / pStack.getUseDuration(pPlayer);
        if (f1 < 0.8F) {
        ...
        pPoseStack.translate(f3 * 0.6F * i, f3 * -0.5F, f3 * 0.0F);
        pPoseStack.mulPose(Axis.YP.rotationDegrees(i * f3 * 90.0F));
    ...
    private void applyBrushTransform(PoseStack pPoseStack, float pPartialTick, HumanoidArm pArm, ItemStack pStack, Player pPlayer, float pEquippedProgress) {
        ...
        float f = pPlayer.getUseItemRemainingTicks() % 10;
    ...
    private void applyItemArmAttackTransform(PoseStack pPoseStack, HumanoidArm pHand, float pSwingProgress) {
        ...
        pPoseStack.mulPose(Axis.YP.rotationDegrees(i * (45.0F + f * -20.0F)));
        ...
        pPoseStack.mulPose(Axis.YP.rotationDegrees(i * -45.0F));
    }

    private void applyItemArmTransform(PoseStack pPoseStack, HumanoidArm pHand, float pEquippedProg) {
        int i = pHand == HumanoidArm.RIGHT ? 1 : -1;
        pPoseStack.translate(i * 0.56F, -0.52F + pEquippedProg * -0.6F, -0.72F);
    }
    ...
    private void renderArmWithItem(
        ...
        if (!pPlayer.isScoping()) {
            ...
            if (pStack.isEmpty()) {
                ...
                int i = flag2 ? 1 : -1;
                if (pPlayer.isUsingItem() && pPlayer.getUseItemRemainingTicks() > 0 && pPlayer.getUsedItemHand() == pHand && !flag1) {
                    this.applyItemArmTransform(pPoseStack, humanoidarm, pEquippedProgress);
                    pPoseStack.translate(i * -0.4785682F, -0.094387F, 0.05731531F);
                    pPoseStack.mulPose(Axis.XP.rotationDegrees(-11.935F));
                    pPoseStack.mulPose(Axis.YP.rotationDegrees(i * 65.3F));
                    pPoseStack.mulPose(Axis.ZP.rotationDegrees(i * -9.785F));
                    float f = pStack.getUseDuration(pPlayer) - (pPlayer.getUseItemRemainingTicks() - pPartialTicks + 1.0F);
                    float f1 = f / CrossbowItem.getChargeDuration(pStack, pPlayer);
                    if (f1 > 1.0F) {
                        f1 = 1.0F;
                    }
                    ...
                    pPoseStack.mulPose(Axis.YN.rotationDegrees(i * 45.0F));
                } else {
                    this.swingArm(pSwingProgress, pEquippedProgress, pPoseStack, i, humanoidarm);
                    if (flag1 && pSwingProgress < 0.001F && flag) {
                        pPoseStack.translate(i * -0.641864F, 0.0F, 0.0F);
                        pPoseStack.mulPose(Axis.YP.rotationDegrees(i * 10.0F));
                    }
                }
                ...
                if (pPlayer.isUsingItem() && pPlayer.getUseItemRemainingTicks() > 0 && pPlayer.getUsedItemHand() == pHand) {
                    switch (pStack.getUseAnimation()) {
                        ...
                        case BLOCK:
                            ...
                            if (!(pStack.getItem() instanceof ShieldItem)) {
                                pPoseStack.translate(j * -0.14142136F, 0.08F, 0.14142136F);
                                pPoseStack.mulPose(Axis.XP.rotationDegrees(-102.25F));
                                pPoseStack.mulPose(Axis.YP.rotationDegrees(j * 13.365F));
                                pPoseStack.mulPose(Axis.ZP.rotationDegrees(j * 78.05F));
                            }
                            break;
                        case BOW:
                            this.applyItemArmTransform(pPoseStack, humanoidarm, pEquippedProgress);
                            pPoseStack.translate(j * -0.2785682F, 0.18344387F, 0.15731531F);
                            pPoseStack.mulPose(Axis.XP.rotationDegrees(-13.935F));
                            pPoseStack.mulPose(Axis.YP.rotationDegrees(j * 35.3F));
                            pPoseStack.mulPose(Axis.ZP.rotationDegrees(j * -9.785F));
                            float f6 = pStack.getUseDuration(pPlayer) - (pPlayer.getUseItemRemainingTicks() - pPartialTicks + 1.0F);
                            float f8 = f6 / 20.0F;
                            ...
                            pPoseStack.mulPose(Axis.YN.rotationDegrees(j * 45.0F));
                            break;
                        case SPEAR:
                            this.applyItemArmTransform(pPoseStack, humanoidarm, pEquippedProgress);
                            pPoseStack.translate(j * -0.5F, 0.7F, 0.1F);
                            pPoseStack.mulPose(Axis.XP.rotationDegrees(-55.0F));
                            pPoseStack.mulPose(Axis.YP.rotationDegrees(j * 35.3F));
                            pPoseStack.mulPose(Axis.ZP.rotationDegrees(j * -9.785F));
                            float f5 = pStack.getUseDuration(pPlayer) - (pPlayer.getUseItemRemainingTicks() - pPartialTicks + 1.0F);
                            float f7 = f5 / 10.0F;
                            if (f7 > 1.0F) {
                                f7 = 1.0F;
                            }
                    ...
                    this.applyItemArmTransform(pPoseStack, humanoidarm, pEquippedProgress);
                    pPoseStack.translate(j * -0.4F, 0.8F, 0.3F);
                    pPoseStack.mulPose(Axis.YP.rotationDegrees(j * 65.0F));
                    pPoseStack.mulPose(Axis.ZP.rotationDegrees(j * -85.0F));
                } else {
                    this.swingArm(pSwingProgress, pEquippedProgress, pPoseStack, j, humanoidarm);
                }
    ...
    private void swingArm(float pSwingProgress, float pEquippedProgress, PoseStack pPoseStack, int pDirection, HumanoidArm pArm) {
        ...
        pPoseStack.translate(pDirection * f, f1, f2);
        this.applyItemArmTransform(pPoseStack, pArm, pEquippedProgress);
