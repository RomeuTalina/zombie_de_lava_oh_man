...
public class TabNavigationBar extends AbstractContainerEventHandler implements Renderable, NarratableEntry {
    ...
    public void selectTab(int pIndex, boolean pPlayClickSound) {
        ...
        } else if (this.tabButtons.get(pIndex).isActive()) {
    ...
    public void setTabActiveState(int pIndex, boolean pActive) {
        if (pIndex >= 0 && pIndex < this.tabButtons.size()) {
            this.tabButtons.get(pIndex).active = pActive;
        }
    }

    public void setTabTooltip(int pIndex, @Nullable Tooltip pTooltip) {
        if (pIndex >= 0 && pIndex < this.tabButtons.size()) {
            this.tabButtons.get(pIndex).setTooltip(pTooltip);
        }
    }
    ...
    private int getNextTabIndex(int pKeycode) {
        return this.getNextTabIndex(this.currentTabIndex(), pKeycode);
    }

    private int getNextTabIndex(int pCurrentTabIndex, int pKeyCode) {
        if (pKeyCode >= 49 && pKeyCode <= 57) {
            return pKeyCode - 49;
        } else if (pKeyCode == 258 && pCurrentTabIndex != -1) {
            int i = Screen.hasShiftDown() ? pCurrentTabIndex - 1 : pCurrentTabIndex + 1;
            int j = Math.floorMod(i, this.tabs.size());
            return this.tabButtons.get(j).active ? j : this.getNextTabIndex(j, pKeyCode);
        } else {
            return -1;
        }
    }
