...
public class SoundEngine {
    ...
    public SoundEngine(MusicManager pMusicManager, SoundManager pSoundManager, Options pOptions, ResourceProvider pResourceManager) {
        this.musicManager = pMusicManager;
    ...
    public void updateCategoryVolume(SoundSource pCategory, float pVolume) {
        if (this.loaded) {
            if (pCategory == SoundSource.MASTER) {
                ...
                if (pCategory == SoundSource.MUSIC && this.options.getSoundSourceVolume(SoundSource.MUSIC) > 0.0F) {
    ...
    public SoundEngine.PlayResult play(SoundInstance pSound) {
        if (!this.loaded) {
            return SoundEngine.PlayResult.NOT_STARTED;
        } else if (!pSound.canPlaySound()) {
            return SoundEngine.PlayResult.NOT_STARTED;
        } else {
            ...
            if (weighedsoundevents == null) {
                ...
                if (sound == SoundManager.INTENTIONALLY_EMPTY_SOUND) {
                    ...
                    boolean flag2 = false;
                    if (f2 == 0.0F) {
                        if (!pSound.canStartSilent() && soundsource != SoundSource.MUSIC) {
                    ...
                    if (this.listener.getGain() <= 0.0F && soundsource != SoundSource.MUSIC) {
                        ...
                        boolean flag3 = shouldLoopAutomatically(pSound);
    ...
    public void pauseAllExcept(SoundSource... pSoundSources) {
        if (this.loaded) {
            for (Entry<SoundInstance, ChannelAccess.ChannelHandle> entry : this.instanceToChannel.entrySet()) {
                if (!List.of(pSoundSources).contains(entry.getKey().getSource())) {
