...
public class PostPass implements AutoCloseable {
    ...
    public PostPass(RenderPipeline pPipeline, ResourceLocation pOutputTargetId, Map<String, List<UniformValue>> pUniforms, List<PostPass.Input> pInputs) {
        ...
        for (Entry<String, List<UniformValue>> entry : pUniforms.entrySet()) {
        ...
        this.infoUbo = new MappableRingBuffer(() -> this.name + " SamplerInfo", 130, (pInputs.size() + 1) * UBO_SIZE_PER_SAMPLER);
    }
        ...
        if (resourcehandle == null) {
            ...
            framepass.executes(
                () -> {
                    ...
                    RenderSystem.setProjectionMatrix(pProjectionMatrixBuffer, ProjectionType.ORTHOGRAPHIC);
                    CommandEncoder commandencoder = RenderSystem.getDevice().createCommandEncoder();
                    List<Pair<String, GpuTextureView>> list = this.inputs
                        .stream()
                        .map(p_404938_ -> Pair.of(p_404938_.samplerName(), p_404938_.texture(pTargets)))
                        .toList();
                    ...
                    for (PostPass.Input postpass$input1 : this.inputs) {
                        postpass$input1.cleanup(pTargets);
                    }
                }
            );
        }
    ...
    public interface Input {
        ...
        GpuTextureView texture(Map<ResourceLocation, ResourceHandle<RenderTarget>> pTargets);
