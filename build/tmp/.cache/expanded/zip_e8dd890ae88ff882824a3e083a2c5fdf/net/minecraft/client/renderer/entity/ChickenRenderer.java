...
public class ChickenRenderer extends MobRenderer<Chicken, ChickenRenderState, ChickenModel> {
    ...
    private static Map<ChickenVariant.ModelType, AdultAndBabyModelPair<ChickenModel>> bakeModels(EntityRendererProvider.Context pContext) {
        return Maps.newEnumMap(
            Map.of(
                ChickenVariant.ModelType.NORMAL,
                new AdultAndBabyModelPair<>(
                    new ChickenModel(pContext.bakeLayer(ModelLayers.CHICKEN)), new ChickenModel(pContext.bakeLayer(ModelLayers.CHICKEN_BABY))
                ),
                ChickenVariant.ModelType.COLD,
                new AdultAndBabyModelPair<>(
                    new ColdChickenModel(pContext.bakeLayer(ModelLayers.COLD_CHICKEN)), new ColdChickenModel(pContext.bakeLayer(ModelLayers.COLD_CHICKEN_BABY))
                )
            )
        );
    }
