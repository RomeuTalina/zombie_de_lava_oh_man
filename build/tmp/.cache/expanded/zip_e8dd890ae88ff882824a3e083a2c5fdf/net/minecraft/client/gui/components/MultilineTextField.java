...
public class MultilineTextField {
    ...
    public void setValue(String pValue, boolean pForce) {
        String s = this.truncateFullText(pValue);
        if (pForce || !this.overflowsLineLimit(s)) {
            this.value = s;
    ...
    private String truncateInsertionText(String pText) {
        ...
        if (this.hasCharacterLimit()) {
            ...
            s = StringUtil.truncateStringIfNecessary(pText, i, false);
        }

        return s;
    }

    private boolean overflowsLineLimit(String pText) {
        return this.hasLineLimit()
            && this.font.getSplitter().splitLines(pText, this.width, Style.EMPTY).size() + (StringUtil.endsWithNewLine(pText) ? 1 : 0)
                > this.lineLimit;
    }
