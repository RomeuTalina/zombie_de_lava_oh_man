...
public class ShulkerBoxRenderer implements BlockEntityRenderer<ShulkerBoxBlockEntity> {
    ...
    public void render(
        ...
        pPoseStack.pushPose();
        this.prepareModel(pPoseStack, pFacing, pProgress);
    ...
    private void prepareModel(PoseStack pPoseStack, Direction pDirection, float pProgress) {
        pPoseStack.translate(0.5F, 0.5F, 0.5F);
        float f = 0.9995F;
        pPoseStack.scale(0.9995F, 0.9995F, 0.9995F);
        pPoseStack.mulPose(pDirection.getRotation());
        pPoseStack.scale(1.0F, -1.0F, -1.0F);
        pPoseStack.translate(0.0F, -1.0F, 0.0F);
        this.model.animate(pProgress);
    }

    public void getExtents(Direction pDirection, float pProgress, Set<Vector3f> pOutput) {
        PoseStack posestack = new PoseStack();
        this.prepareModel(posestack, pDirection, pProgress);
        this.model.root().getExtentsForGui(posestack, pOutput);
    }
