...
public class ClientBundleTooltip implements ClientTooltipComponent {
    ...
    private void renderSlot(int pSlotIndex, int pX, int pY, List<ItemStack> pShownItems, int pSeed, Font pFont, GuiGraphics pGuiGraphics) {
        ...
        if (flag) {
            pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, SLOT_HIGHLIGHT_BACK_SPRITE, pX, pY, 24, 24);
        } else {
            pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, SLOT_BACKGROUND_SPRITE, pX, pY, 24, 24);
        }

        pGuiGraphics.renderItem(itemstack, pX + 4, pY + 4, pSeed);
        pGuiGraphics.renderItemDecorations(pFont, itemstack, pX + 4, pY + 4);
        if (flag) {
            pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, SLOT_HIGHLIGHT_FRONT_SPRITE, pX, pY, 24, 24);
        }
    }

    private static void renderCount(int pSlotX, int pSlotY, int pCount, Font pFont, GuiGraphics pGuiGraphics) {
        pGuiGraphics.drawCenteredString(pFont, "+" + pCount, pSlotX + 12, pSlotY + 10, -1);
    }
    ...
    private void drawProgressbar(int pX, int pY, Font pFont, GuiGraphics pGuiGraphics) {
        pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, this.getProgressBarTexture(), pX + 1, pY, this.getProgressBarFill(), 13);
        pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, PROGRESSBAR_BORDER_SPRITE, pX, pY, 96, 13);
        Component component = this.getProgressBarFillText();
        if (component != null) {
            pGuiGraphics.drawCenteredString(pFont, component, pX + 48, pY + 3, -1);
        }
    }

    private static void drawEmptyBundleDescriptionText(int pX, int pY, Font pFont, GuiGraphics pGuiGraphics) {
        pGuiGraphics.drawWordWrap(pFont, BUNDLE_EMPTY_DESCRIPTION, pX, pY, 96, -5592406);
    }
