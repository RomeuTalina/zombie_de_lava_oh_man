...
public class PigRenderer extends MobRenderer<Pig, PigRenderState, PigModel> {
    ...
    private static Map<PigVariant.ModelType, AdultAndBabyModelPair<PigModel>> bakeModels(EntityRendererProvider.Context pContext) {
        return Maps.newEnumMap(
            Map.of(
                PigVariant.ModelType.NORMAL,
                new AdultAndBabyModelPair<>(new PigModel(pContext.bakeLayer(ModelLayers.PIG)), new PigModel(pContext.bakeLayer(ModelLayers.PIG_BABY))),
                PigVariant.ModelType.COLD,
                new AdultAndBabyModelPair<>(
                    new ColdPigModel(pContext.bakeLayer(ModelLayers.COLD_PIG)), new ColdPigModel(pContext.bakeLayer(ModelLayers.COLD_PIG_BABY))
                )
            )
        );
    }
