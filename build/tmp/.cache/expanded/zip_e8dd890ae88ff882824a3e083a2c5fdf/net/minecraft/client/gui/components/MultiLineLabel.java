...
public interface MultiLineLabel {
                ...
                public void renderCentered(GuiGraphics p_281603_, int p_281267_, int p_281819_, int p_281545_, int p_282780_) {
                    ...
                    for (MultiLineLabel.TextAndWidth multilinelabel$textandwidth : this.getSplitMessage()) {
                        p_281603_.drawString(
                            pFont, multilinelabel$textandwidth.text, p_281267_ - multilinelabel$textandwidth.width / 2, i, p_282780_
                        );
                        i += p_281545_;
                    }
                }
                ...
                public Style getStyleAtCentered(int p_407426_, int p_409697_, int p_410519_, double p_406056_, double p_406328_) {
                    ...
                    if (i >= 0 && i < list.size()) {
                        ...
                        if (p_406056_ < j) {
                            ...
                            return pFont.getSplitter().componentStyleAtWidth(multilinelabel$textandwidth.text, k);
                        }
                    } else {
                        return null;
                    }
                ...
                public Style getStyleAtLeftAligned(int p_407554_, int p_407233_, int p_405911_, double p_410696_, double p_407790_) {
                    if (p_410696_ < p_407554_) {
                        ...
                        if (i >= 0 && i < list.size()) {
                            ...
                            return pFont.getSplitter().componentStyleAtWidth(multilinelabel$textandwidth.text, j);
                        } else {
                            return null;
                        }
                    }
                ...
                private List<MultiLineLabel.TextAndWidth> getSplitMessage() {
                    ...
                    if (this.cachedTextAndWidth != null && language == this.splitWithLanguage) {
                        ...
                        for (Component component : pComponents) {
                            list.addAll(pFont.splitIgnoringLanguage(component, pMaxWidth));
                        }

                        this.cachedTextAndWidth = new ArrayList<>();
                        int i = Math.min(list.size(), pMaxRows);
                        ...
                        for (int j = 0; j < list1.size(); j++) {
                            ...
                            if (j == list1.size() - 1 && i == pMaxRows && i != list.size()) {
                                FormattedText formattedtext = pFont.substrByWidth(
                                    formattedtext2, pFont.width(formattedtext2) - pFont.width(CommonComponents.ELLIPSIS)
                                );
                                FormattedText formattedtext1 = FormattedText.composite(formattedtext, CommonComponents.ELLIPSIS);
                                this.cachedTextAndWidth
                                    .add(new MultiLineLabel.TextAndWidth(Language.getInstance().getVisualOrder(formattedtext1), pFont.width(formattedtext1)));
                            } else {
    ...
    @Nullable
    Style getStyleAtCentered(int pX, int pY, int pHeight, double pMouseX, double pMouseY);

    @Nullable
    Style getStyleAtLeftAligned(int pX, int pY, int pHeight, double pMouseX, double pMouseY);
