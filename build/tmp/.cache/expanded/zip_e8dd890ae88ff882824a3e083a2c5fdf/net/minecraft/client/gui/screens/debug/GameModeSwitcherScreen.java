...
public class GameModeSwitcherScreen extends Screen {
    ...
    private static void switchToHoveredGameMode(Minecraft pMinecraft, GameModeSwitcherScreen.GameModeIcon pGameModeIcon) {
        if (pMinecraft.gameMode != null && pMinecraft.player != null) {
            ...
            if (pMinecraft.player.hasPermissions(2) && pGameModeIcon != gamemodeswitcherscreen$gamemodeicon) {
                pMinecraft.player.connection.send(new ServerboundChangeGameModePacket(pGameModeIcon.mode));
            }
        }
    }
    ...
    static enum GameModeIcon {
        ...
        private GameModeIcon(final Component pName, final GameType pMode, final ItemStack pRenderStack) {
            this.name = pName;
            this.mode = pMode;
    ...
    public static class GameModeSlot extends AbstractWidget {
        ...
        private void drawSlot(GuiGraphics pGuiGraphics) {
            pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, GameModeSwitcherScreen.SLOT_SPRITE, this.getX(), this.getY(), 26, 26);
        }

        private void drawSelection(GuiGraphics pGuiGraphics) {
            pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, GameModeSwitcherScreen.SELECTION_SPRITE, this.getX(), this.getY(), 26, 26);
        }
    }
}