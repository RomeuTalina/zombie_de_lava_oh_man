...
public class PlayerHeadSpecialRenderer implements SpecialModelRenderer<PlayerHeadSpecialRenderer.PlayerHeadRenderInfo> {
    ...
    PlayerHeadSpecialRenderer(SkinManager pSkinManager, SkullModelBase pModelBase, PlayerHeadSpecialRenderer.PlayerHeadRenderInfo pDefaultPlayerHeadRenderInfo) {
        this.skinManager = pSkinManager;
        this.modelBase = pModelBase;
        this.defaultPlayerHeadRenderInfo = pDefaultPlayerHeadRenderInfo;
    }
    ...
    @Nullable
    private PlayerHeadSpecialRenderer.PlayerHeadRenderInfo createAndCacheIfTextureIsUnpacked(ResolvableProfile pProfile) {
        PlayerSkin playerskin = this.skinManager.getInsecureSkin(pProfile.gameProfile(), null);
        if (playerskin != null) {
            ...
            this.updatedResolvableProfiles.put(pProfile, playerheadspecialrenderer$playerheadrenderinfo);
            return playerheadspecialrenderer$playerheadrenderinfo;
        } else {
            return null;
        }
    ...
    public record PlayerHeadRenderInfo(RenderType renderType) {
        static PlayerHeadSpecialRenderer.PlayerHeadRenderInfo create(PlayerSkin pSkin) {
            return new PlayerHeadSpecialRenderer.PlayerHeadRenderInfo(SkullBlockRenderer.getPlayerSkinRenderType(pSkin.texture()));
        }
    }
