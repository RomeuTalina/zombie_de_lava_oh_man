...
public class ModelBakery {
    ...
    public ModelBakery(
        ...
        Map<BlockState, BlockStateModel.UnbakedRoot> pUnbakedBlockStateModels,
        Map<ResourceLocation, ClientItem> pClientInfos,
        Map<ResourceLocation, ResolvedModel> pResolvedModels,
        ResolvedModel pMissingModel
    ) {
        this.entityModelSet = pEntityModelSet;
        this.unbakedBlockStateModels = pUnbakedBlockStateModels;
        this.clientInfos = pClientInfos;
        this.resolvedModels = pResolvedModels;
    ...
    public CompletableFuture<ModelBakery.BakingResult> bakeModels(SpriteGetter pSprites, Executor pExecutor) {
        ModelBakery.MissingModels modelbakery$missingmodels = ModelBakery.MissingModels.bake(this.missingModel, pSprites);
        ModelBakery.ModelBakerImpl modelbakery$modelbakerimpl = new ModelBakery.ModelBakerImpl(pSprites);
    ...
    public record MissingModels(BlockStateModel block, ItemModel item) {
        public static ModelBakery.MissingModels bake(ResolvedModel pModel, final SpriteGetter pSprites) {
            ...
            TextureSlots textureslots = pModel.getTopTextureSlots();
            boolean flag = pModel.getTopAmbientOcclusion();
            boolean flag1 = pModel.getTopGuiLight().lightLikeBlock();
            ItemTransforms itemtransforms = pModel.getTopTransforms();
            QuadCollection quadcollection = pModel.bakeTopGeometry(textureslots, modelbaker, BlockModelRotation.X0_Y0);
            TextureAtlasSprite textureatlassprite = pModel.resolveParticleSprite(textureslots, modelbaker);
    ...
    class ModelBakerImpl implements ModelBaker {
        ...
        ModelBakerImpl(final SpriteGetter pSprites) {
