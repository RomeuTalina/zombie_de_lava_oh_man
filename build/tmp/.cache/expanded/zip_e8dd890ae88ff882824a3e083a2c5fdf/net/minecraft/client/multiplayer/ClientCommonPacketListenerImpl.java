...
public abstract class ClientCommonPacketListenerImpl implements ClientCommonPacketListener {
    ...
    public void showDialog(Holder<Dialog> pDialog, @Nullable Screen pPreviousScreen) {
        this.showDialog(pDialog, this.createDialogAccess(), pPreviousScreen);
    }

    protected void showDialog(Holder<Dialog> pDialog, DialogConnectionAccess pConnectionAccess, @Nullable Screen pPreviousScreen) {
        if (pPreviousScreen instanceof DialogScreen.WarningScreen dialogscreen$warningscreen) {
            ...
            DialogScreen<?> dialogscreen2 = DialogScreens.createFromData(pDialog.value(), screen3, pConnectionAccess);
            if (dialogscreen2 != null) {
                dialogscreen$warningscreen.updateReturnScreen(dialogscreen2);
            } else {
                LOGGER.warn("Failed to show dialog for data {}", pDialog);
            }
        } else {
            Screen screen;
            if (pPreviousScreen instanceof DialogScreen<?> dialogscreen) {
                screen = dialogscreen.previousScreen();
            } else if (pPreviousScreen instanceof WaitingForResponseScreen waitingforresponsescreen) {
                screen = waitingforresponsescreen.previousScreen();
            } else {
                screen = pPreviousScreen;
            }

            Screen screen1 = DialogScreens.createFromData(pDialog.value(), screen, pConnectionAccess);
            if (screen1 != null) {
    ...
    protected Screen createDisconnectScreen(DisconnectionDetails pDetails) {
        ...
        return this.serverData != null && this.serverData.isRealm()
            ? new DisconnectedScreen(screen, GENERIC_DISCONNECT_MESSAGE, pDetails, CommonComponents.GUI_BACK)
