...
public class CowRenderer extends MobRenderer<Cow, CowRenderState, CowModel> {
    ...
    private static Map<CowVariant.ModelType, AdultAndBabyModelPair<CowModel>> bakeModels(EntityRendererProvider.Context pContext) {
        return Maps.newEnumMap(
            Map.of(
                CowVariant.ModelType.NORMAL,
                new AdultAndBabyModelPair<>(new CowModel(pContext.bakeLayer(ModelLayers.COW)), new CowModel(pContext.bakeLayer(ModelLayers.COW_BABY))),
                CowVariant.ModelType.WARM,
                new AdultAndBabyModelPair<>(new CowModel(pContext.bakeLayer(ModelLayers.WARM_COW)), new CowModel(pContext.bakeLayer(ModelLayers.WARM_COW_BABY))),
                CowVariant.ModelType.COLD,
                new AdultAndBabyModelPair<>(new CowModel(pContext.bakeLayer(ModelLayers.COLD_COW)), new CowModel(pContext.bakeLayer(ModelLayers.COLD_COW_BABY)))
            )
        );
    }
