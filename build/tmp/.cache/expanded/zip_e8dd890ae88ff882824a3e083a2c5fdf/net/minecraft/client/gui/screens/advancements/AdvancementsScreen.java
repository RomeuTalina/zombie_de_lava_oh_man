...
public class AdvancementsScreen extends Screen implements ClientAdvancements.Listener {
    ...
    private void renderInside(GuiGraphics pGuiGraphics, int pX, int pY) {
        AdvancementTab advancementtab = this.selectedTab;
        if (advancementtab == null) {
            pGuiGraphics.fill(pX + 9, pY + 18, pX + 9 + 234, pY + 18 + 113, -16777216);
            int i = pX + 9 + 117;
            pGuiGraphics.drawCenteredString(this.font, NO_ADVANCEMENTS_LABEL, i, pY + 18 + 56 - 9 / 2, -1);
            pGuiGraphics.drawCenteredString(this.font, VERY_SAD_LABEL, i, pY + 18 + 113 - 9, -1);
        } else {
            advancementtab.drawContents(pGuiGraphics, pX + 9, pY + 18);
        }
    }

    public void renderWindow(GuiGraphics pGuiGraphics, int pOffsetX, int pOffsetY) {
        pGuiGraphics.blit(RenderPipelines.GUI_TEXTURED, WINDOW_LOCATION, pOffsetX, pOffsetY, 0.0F, 0.0F, 252, 140, 256, 256);
        if (this.tabs.size() > 1) {
            for (AdvancementTab advancementtab : this.tabs.values()) {
        ...
        pGuiGraphics.drawString(this.font, this.selectedTab != null ? this.selectedTab.getTitle() : TITLE, pOffsetX + 8, pOffsetY + 6, -12566464, false);
    }

    private void renderTooltips(GuiGraphics pGuiGraphics, int pMouseX, int pMouseY, int pOffsetX, int pOffsetY) {
        if (this.selectedTab != null) {
            pGuiGraphics.pose().pushMatrix();
            pGuiGraphics.pose().translate(pOffsetX + 9, pOffsetY + 18);
        ...
        if (this.tabs.size() > 1) {
            for (AdvancementTab advancementtab : this.tabs.values()) {
                ...
                if (advancementtab.isMouseOver(pOffsetX, pOffsetY, pMouseX, pMouseY)) {
                    pGuiGraphics.setTooltipForNextFrame(this.font, advancementtab.getTitle(), pMouseX, pMouseY);
                }
            }
        }
    }
