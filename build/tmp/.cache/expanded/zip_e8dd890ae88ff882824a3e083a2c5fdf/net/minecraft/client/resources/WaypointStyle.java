...
public record WaypointStyle(int nearDistance, int farDistance, List<ResourceLocation> sprites, List<ResourceLocation> spriteLocations) {
    ...
    public WaypointStyle(int pNearDistance, int pFarDistance, List<ResourceLocation> pSprites) {
        this(pNearDistance, pFarDistance, pSprites, pSprites.stream().map(p_409360_ -> p_409360_.withPrefix("hud/locator_bar_dot/")).toList());
    }
    ...
    public ResourceLocation sprite(float pDistance) {
        if (pDistance <= this.nearDistance) {
            return this.spriteLocations.getFirst();
        } else if (pDistance >= this.farDistance) {
            return this.spriteLocations.getLast();
        } else {
            int i = Mth.lerpInt((pDistance - this.nearDistance) / (this.farDistance - this.nearDistance), 0, this.spriteLocations.size());
