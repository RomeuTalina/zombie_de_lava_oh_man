...
public class WidgetTooltipHolder {
    ...
    public void refreshTooltipForNextRenderPass(GuiGraphics pGuiGraphics, int pMouseX, int pMouseY, boolean pHovering, boolean pFocused, ScreenRectangle pScreenRectangle) {
        if (this.tooltip == null) {
            ...
            boolean flag = pHovering || pFocused && minecraft.getLastInputType().isKeyboard();
            ...
            if (flag && Util.getMillis() - this.displayStartTime > this.delay.toMillis()) {
                pGuiGraphics.setTooltipForNextFrame(
                    minecraft.font, this.tooltip.toCharSequence(minecraft), this.createTooltipPositioner(pScreenRectangle, pHovering, pFocused), pMouseX, pMouseY, pFocused
                );
            }
        }
    }
