...
public record Selector(Optional<Condition> condition, BlockStateModel.Unbaked variant) {
    ...
    public <O, S extends StateHolder<O, S>> Predicate<S> instantiate(StateDefinition<O, S> pStateDefinition) {
        return this.condition.<Predicate<S>>map(p_393307_ -> p_393307_.instantiate(pStateDefinition)).orElse(p_392027_ -> true);
    }
}