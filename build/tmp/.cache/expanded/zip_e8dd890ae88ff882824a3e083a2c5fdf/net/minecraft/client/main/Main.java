...
public class Main {
    ...
    private static GameConfig.QuickPlayVariant getQuickPlayVariant(
        OptionSet pOptions, OptionSpec<String> pSingleplayer, OptionSpec<String> pMultiplayer, OptionSpec<String> pRealms
    ) {
        long i = Stream.of(pSingleplayer, pMultiplayer, pRealms).filter(pOptions::has).count();
        if (i == 0L) {
        ...
        } else if (pOptions.has(pSingleplayer)) {
            String s2 = unescapeJavaArgument(parseArgument(pOptions, pSingleplayer));
            return new GameConfig.QuickPlaySinglePlayerData(s2);
        } else if (pOptions.has(pMultiplayer)) {
            String s1 = unescapeJavaArgument(parseArgument(pOptions, pMultiplayer));
            return Optionull.mapOrDefault(s1, GameConfig.QuickPlayMultiplayerData::new, GameConfig.QuickPlayVariant.DISABLED);
        } else if (pOptions.has(pRealms)) {
            String s = unescapeJavaArgument(parseArgument(pOptions, pRealms));
