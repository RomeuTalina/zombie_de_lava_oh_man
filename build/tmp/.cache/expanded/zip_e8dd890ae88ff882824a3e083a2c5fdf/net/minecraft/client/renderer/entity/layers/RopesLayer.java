...
public class RopesLayer<M extends HappyGhastModel> extends RenderLayer<HappyGhastRenderState, M> {
    ...
    public RopesLayer(RenderLayerParent<HappyGhastRenderState, M> pRenderer, EntityModelSet pEntityModels, ResourceLocation pTexture) {
        super(pRenderer);
        this.ropes = RenderType.entityCutoutNoCull(pTexture);
        this.adultModel = new HappyGhastModel(pEntityModels.bakeLayer(ModelLayers.HAPPY_GHAST_ROPES));
        this.babyModel = new HappyGhastModel(pEntityModels.bakeLayer(ModelLayers.HAPPY_GHAST_BABY_ROPES));
    }
