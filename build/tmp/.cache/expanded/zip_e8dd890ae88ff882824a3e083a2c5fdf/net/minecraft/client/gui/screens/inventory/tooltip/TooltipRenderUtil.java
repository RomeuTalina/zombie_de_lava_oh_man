...
public class TooltipRenderUtil {
    ...
    public static void renderTooltipBackground(GuiGraphics pGuiGraphics, int pX, int pY, int pWidth, int pHeight, @Nullable ResourceLocation pSprite) {
        int i = pX - 3 - 9;
        int j = pY - 3 - 9;
        int k = pWidth + 3 + 3 + 18;
        int l = pHeight + 3 + 3 + 18;
        pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, getBackgroundSprite(pSprite), i, j, k, l);
        pGuiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, getFrameSprite(pSprite), i, j, k, l);
    }
