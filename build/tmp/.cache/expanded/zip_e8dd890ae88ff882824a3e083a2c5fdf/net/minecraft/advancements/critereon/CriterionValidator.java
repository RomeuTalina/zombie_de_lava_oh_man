...
public class CriterionValidator {
    ...
    public void validate(ContextAwarePredicate pEntity, ContextKeySet pContextKeySet, String pName) {
        pEntity.validate(new ValidationContext(this.reporter.forChild(new ProblemReporter.FieldPathElement(pName)), pContextKeySet, this.lootData));
    }
    ...
    public void validate(List<ContextAwarePredicate> pEntities, ContextKeySet pContextKeySet, String pName) {
        for (int i = 0; i < pEntities.size(); i++) {
            ...
            contextawarepredicate.validate(
                new ValidationContext(this.reporter.forChild(new ProblemReporter.IndexedFieldPathElement(pName, i)), pContextKeySet, this.lootData)
            );
        }
    }
}