...
public class Commands {
    ...
    public Commands(Commands.CommandSelection pSelection, CommandBuildContext pContext) {
        ...
        DataPackCommand.register(this.dispatcher, pContext);
        DebugCommand.register(this.dispatcher);
        DefaultGameModeCommands.register(this.dispatcher);
        DialogCommand.register(this.dispatcher, pContext);
        ...
        VersionCommand.register(this.dispatcher, pSelection != Commands.CommandSelection.INTEGRATED);
        ...
        WaypointCommand.register(this.dispatcher, pContext);
        ...
        if (net.minecraftforge.gametest.ForgeGameTestHooks.isGametestEnabled()) {
            ...
            if (pSelection.includeDedicated) {
                DebugConfigCommand.register(this.dispatcher, pContext);
            }
        }
    ...
    public void performPrefixedCommand(CommandSourceStack pSource, String pCommand) {
        pCommand = trimOptionalPrefix(pCommand);
        this.performCommand(this.dispatcher.parse(pCommand, pSource), pCommand);
    }

    public static String trimOptionalPrefix(String pCommand) {
        return pCommand.startsWith("/") ? pCommand.substring(1) : pCommand;
    }
    ...
    public void sendCommands(ServerPlayer pPlayer) {
        ...
        net.minecraftforge.server.command.CommandHelper.mergeCommandNode(this.dispatcher.getRoot(), rootcommandnode, map, pPlayer.createCommandSourceStack(), ctx -> 0, suggest -> suggest);
        // FORGE: Clean any modded command content if the client is vanilla
        rootcommandnode = net.minecraftforge.server.command.CommandHelper.filterCommandList(pPlayer.connection.getConnection(), rootcommandnode);
        pPlayer.connection.send(new ClientboundCommandsPacket(rootcommandnode, COMMAND_NODE_INSPECTOR));
    }

    private static <S> void fillUsableCommands(CommandNode<S> pRoot, CommandNode<S> pCurrent, S pSource, Map<CommandNode<S>, CommandNode<S>> pOutput) {
        for (CommandNode<S> commandnode : pRoot.getChildren()) {
            if (commandnode.canUse(pSource)) {
                ArgumentBuilder<S, ?> argumentbuilder = commandnode.createBuilder();
                if (argumentbuilder.getRedirect() != null) {
                    argumentbuilder.redirect(pOutput.get(argumentbuilder.getRedirect()));
                }

                CommandNode<S> commandnode1 = argumentbuilder.build();
                pOutput.put(commandnode, commandnode1);
                pCurrent.addChild(commandnode1);
                if (!commandnode.getChildren().isEmpty()) {
                    fillUsableCommands(commandnode, commandnode1, pSource, pOutput);
                }
            }
        }
    }
    ...
    public static CommandBuildContext createValidationContext(final HolderLookup.Provider pProvider) {
        return new CommandBuildContext() {
            ...
            public <T> Optional<HolderLookup.RegistryLookup<T>> lookup(ResourceKey<? extends Registry<? extends T>> p_407321_) {
                return pProvider.lookup(p_407321_).map(this::createLookup);
            }
    ...
    public static <T extends PermissionSource> PermissionCheck<T> hasPermission(int pLevel) {
        return new PermissionSource.Check<>(pLevel);
    }
