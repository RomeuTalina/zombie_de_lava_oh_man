...
public class ItemPredicateArgument extends ParserBasedArgument<ItemPredicateArgument.Result> {
    ...
    public ItemPredicateArgument(CommandBuildContext pContext) {
        super(ComponentPredicateParser.createGrammar(new ItemPredicateArgument.Context(pContext)).mapResult(p_389646_ -> Util.allOf(p_389646_)::test));
    }
    ...
    record ComponentWrapper(ResourceLocation id, Predicate<ItemStack> presenceChecker, Decoder<? extends Predicate<ItemStack>> valueChecker) {
        public static <T> ItemPredicateArgument.ComponentWrapper create(
            ...
            if (codec == null) {
                ...
                return new ItemPredicateArgument.ComponentWrapper(pId, p_389648_ -> p_389648_.has(pComponentType), codec.map(p_335085_ -> p_331446_ -> {
        ...
        public Predicate<ItemStack> decode(ImmutableStringReader pReader, Dynamic<?> pData) throws CommandSyntaxException {
            DataResult<? extends Predicate<ItemStack>> dataresult = this.valueChecker.parse(pData);
    ...
    static class Context
        ...
        Context(HolderLookup.Provider pRegistries) {
            ...
            this.predicates = pRegistries.lookupOrThrow(Registries.DATA_COMPONENT_PREDICATE_TYPE);
        }
    ...
    record PredicateWrapper(ResourceLocation id, Decoder<? extends Predicate<ItemStack>> type) {
        public PredicateWrapper(Holder.Reference<DataComponentPredicate.Type<?>> pPredicate) {
            this(pPredicate.key().location(), pPredicate.value().codec().map(p_394448_ -> p_394448_::matches));
        }

        public Predicate<ItemStack> decode(ImmutableStringReader pReader, Dynamic<?> pData) throws CommandSyntaxException {
            DataResult<? extends Predicate<ItemStack>> dataresult = this.type.parse(pData);
            return (Predicate<ItemStack>)dataresult.getOrThrow(
