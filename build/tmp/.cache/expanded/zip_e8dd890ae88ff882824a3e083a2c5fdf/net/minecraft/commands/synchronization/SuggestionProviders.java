...
public class SuggestionProviders {
    ...
    public static <S extends SharedSuggestionProvider> SuggestionProvider<S> register(
        ...
        SuggestionProvider<SharedSuggestionProvider> suggestionprovider = PROVIDERS_BY_NAME.putIfAbsent(pName, pProvider);
        if (suggestionprovider != null) {
            throw new IllegalArgumentException("A command suggestion provider is already registered with the name '" + pName + "'");
        } else {
            return (SuggestionProvider<S>)new SuggestionProviders.RegisteredSuggestion(pName, pProvider);
        }
    }

    public static <S extends SharedSuggestionProvider> SuggestionProvider<S> cast(SuggestionProvider<SharedSuggestionProvider> pProvider) {
        return (SuggestionProvider<S>)pProvider;
    }

    public static <S extends SharedSuggestionProvider> SuggestionProvider<S> getProvider(ResourceLocation pName) {
        return cast(PROVIDERS_BY_NAME.getOrDefault(pName, ASK_SERVER));
    }

    public static ResourceLocation getName(SuggestionProvider<?> pProvider) {
        return pProvider instanceof SuggestionProviders.RegisteredSuggestion suggestionproviders$registeredsuggestion
