...
public interface SharedSuggestionProvider {
    ...
    default void suggestRegistryElements(HolderLookup<?> pLookup, SharedSuggestionProvider.ElementSuggestionType pType, SuggestionsBuilder pBuilder) {
        if (pType.shouldSuggestTags()) {
            suggestResource(pLookup.listTagIds().map(TagKey::location), pBuilder, "#");
        }

        if (pType.shouldSuggestElements()) {
            suggestResource(pLookup.listElementIds().map(ResourceKey::location), pBuilder);
        }
    }

    static <S> CompletableFuture<Suggestions> listSuggestions(
        CommandContext<S> pContext,
        SuggestionsBuilder pBuilder,
        ResourceKey<? extends Registry<?>> pRegistryKey,
        SharedSuggestionProvider.ElementSuggestionType pType
    ) {
        return pContext.getSource() instanceof SharedSuggestionProvider sharedsuggestionprovider
            ? sharedsuggestionprovider.suggestRegistryElements(pRegistryKey, pType, pBuilder, pContext)
    ...
    static <T> void filterResources(Iterable<T> pResources, String pInput, Function<T, ResourceLocation> pLocationFunction, Consumer<T> pResourceConsumer) {
        ...
        for (T t : pResources) {
            ...
            if (flag) {
                ...
                || matchesSubStr(pInput, resourcelocation.getPath())) { // Forge: Removed "minecraft" namespace requirement for path searching to allow modded path suggestions to show up
                pResourceConsumer.accept(t);
            }
        }
    }
