...
public class BlockMath {
    ...
    public static Transformation getFaceTransformation(Transformation pTransformation, Direction pDirection) {
        if (MatrixUtil.isIdentity(pTransformation.getMatrix())) {
            return pTransformation;
        } else {
            Transformation transformation = VANILLA_UV_TRANSFORM_LOCAL_TO_GLOBAL.get(pDirection);
            transformation = pTransformation.compose(transformation);
