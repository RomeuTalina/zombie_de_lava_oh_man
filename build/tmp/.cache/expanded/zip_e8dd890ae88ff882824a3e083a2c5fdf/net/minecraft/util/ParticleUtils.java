...
public class ParticleUtils {
    ...
    public static void spawnParticleOnFace(Level pLevel, BlockPos pPos, Direction pDirection, ParticleOptions pParticle, Vec3 pSpeed, double pSpread) {
        ...
        double d0 = vec3.x + (i == 0 ? Mth.nextDouble(pLevel.random, -0.5, 0.5) : i * pSpread);
        double d1 = vec3.y + (j == 0 ? Mth.nextDouble(pLevel.random, -0.5, 0.5) : j * pSpread);
        double d2 = vec3.z + (k == 0 ? Mth.nextDouble(pLevel.random, -0.5, 0.5) : k * pSpread);
    ...
    public static void spawnParticles(
        ...
        for (int i = 0; i < pCount; i++) {
            ...
            double d4 = pPos.getX() + d3 + randomsource.nextDouble() * pXzSpread * 2.0;
            double d5 = pPos.getY() + randomsource.nextDouble() * pYSpread;
            double d6 = pPos.getZ() + d3 + randomsource.nextDouble() * pXzSpread * 2.0;
    ...
    public static void spawnSmashAttackParticles(LevelAccessor pLevel, BlockPos pPos, int pPower) {
        ...
        for (int j = 0; j < pPower / 1.5F; j++) {
            double d6 = vec3.x + 3.5 * Math.cos(j) + pLevel.getRandom().nextGaussian() / 2.0;
            double d7 = vec3.y;
            double d8 = vec3.z + 3.5 * Math.sin(j) + pLevel.getRandom().nextGaussian() / 2.0;
