...
public class ParticleUnflatteningFix extends DataFix {
    ...
    private <T> Dynamic<T> updateItem(Dynamic<T> pTag, String pItem) {
        ...
        if (i == -1) {
            ...
            Dynamic<T> dynamic1 = parseTag(pTag.getOps(), pItem.substring(i));
            if (dynamic1 != null) {
    ...
    @Nullable
    private static <T> Dynamic<T> parseTag(DynamicOps<T> pOps, String pTag) {
        try {
            return new Dynamic<>(pOps, TagParser.create(pOps).parseFully(pTag));
