...
public class ItemStackCustomNameToOverrideComponentFix extends DataFix {
    ...
    private static <T> Typed<T> fixMap(Typed<T> pData) {
        return fixCustomName(pData, MAP_NAMES::contains);
    }

    private static <T> Typed<T> fixBanner(Typed<T> pData) {
        return fixCustomName(pData, p_329571_ -> p_329571_.equals("block.minecraft.ominous_banner"));
    }

    private static <T> Typed<T> fixCustomName(Typed<T> pData, Predicate<String> pNamePredicate) {
        return Util.writeAndReadTypedOrThrow(pData, pData.getType(), p_390289_ -> {
            OptionalDynamic<?> optionaldynamic = p_390289_.get("minecraft:custom_name");
            Optional<String> optional = optionaldynamic.asString().result().flatMap(LegacyComponentDataFixUtils::extractTranslationString).filter(pNamePredicate);
