...
public class Mth {
    ...
    public static int smallestSquareSide(int pItemCount) {
        if (pItemCount < 0) {
            ...
            return ceil(Math.sqrt(pItemCount));
        }
    }
    ...
    public static Quaternionf rotationAroundAxis(Vector3f pAxis, Quaternionf pCameraOrientation, Quaternionf pOutput) {
        float f = pAxis.dot(pCameraOrientation.x, pCameraOrientation.y, pCameraOrientation.z);
        return pOutput.set(pAxis.x * f, pAxis.y * f, pAxis.z * f, pCameraOrientation.w).normalize();
    }
