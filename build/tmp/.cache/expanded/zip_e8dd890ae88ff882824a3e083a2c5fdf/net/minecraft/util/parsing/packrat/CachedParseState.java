...
public abstract class CachedParseState<S> implements ParseState<S> {
    ...
    protected CachedParseState(ErrorCollector<S> pErrorCollector) {
    ...
    private CachedParseState.PositionCache getCacheForPosition(int pPosition) {
        int i = this.positionCache.length;
        if (pPosition >= i) {
            int j = Util.growByHalf(i, pPosition + 1);
        ...
        CachedParseState.PositionCache cachedparsestate$positioncache = this.positionCache[pPosition];
        if (cachedparsestate$positioncache == null) {
            cachedparsestate$positioncache = new CachedParseState.PositionCache();
            this.positionCache[pPosition] = cachedparsestate$positioncache;
        }
    ...
    static class PositionCache {
        ...
        public int findKeyIndex(Atom<?> pAtom) {
            for (int i = 0; i < this.nextKey; i += 2) {
                if (this.atomCache[i] == pAtom) {
                    return i;
                }
            }
        ...
        public int allocateNewEntry(Atom<?> pEntry) {
            int i = this.nextKey;
            this.nextKey += 2;
            int j = i + 1;
            ...
            this.atomCache[i] = pEntry;
            return i;
        }

        @Nullable
        public <T> CachedParseState.CacheEntry<T> getValue(int pIndex) {
            return (CachedParseState.CacheEntry<T>)this.atomCache[pIndex + 1];
        }

        public void setValue(int pIndex, CachedParseState.CacheEntry<?> pValue) {
            this.atomCache[pIndex + 1] = pValue;
        }
    }
