...
public class BannerEntityCustomNameToOverrideComponentFix extends DataFix {
    ...
    private Typed<?> fix(Typed<?> pData, OpticFinder<Pair<String, String>> pTextComponentTypeOptic, OpticFinder<?> pCustomNameOptic) {
        Optional<String> optional = pData.getOptionalTyped(pCustomNameOptic).flatMap(p_390218_ -> p_390218_.getOptional(pTextComponentTypeOptic).map(Pair::getSecond));
