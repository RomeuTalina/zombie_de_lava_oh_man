...
public class GsonHelper {
    ...
    public static <T> T fromJson(Gson pGson, Reader pReader, Class<T> pJsonClass) {
        try {
            ...
            T t = pGson.getAdapter(pJsonClass).read(jsonreader);
            if (t == null) {
    ...
    @Nullable
    public static <T> T fromNullableJson(Gson pGson, Reader pReader, TypeToken<T> pToken) {
        try {
            JsonReader jsonreader = new JsonReader(pReader);
            jsonreader.setStrictness(Strictness.STRICT);
            return pGson.getAdapter(pToken).read(jsonreader);
        } catch (IOException ioexception) {
            throw new JsonParseException(ioexception);
        }
    }
    ...
    public static <T> T fromJson(Gson pGson, Reader pReader, TypeToken<T> pType) {
        T t = fromNullableJson(pGson, pReader, pType);
        if (t == null) {
            throw new JsonParseException("JSON data was null or empty");
        } else {
            return t;
    ...
    public static <T> T fromNullableJson(Gson pGson, String pJson, TypeToken<T> pType) {
        return fromNullableJson(pGson, new StringReader(pJson), pType);
    }

    public static <T> T fromJson(Gson pGson, String pJson, Class<T> pAdapter) {
        return fromJson(pGson, new StringReader(pJson), pAdapter);
    }

    public static JsonObject parse(String pJson) {
        return parse(new StringReader(pJson));
    }

    public static JsonObject parse(Reader pReader) {
        return fromJson(GSON, pReader, JsonObject.class);
    }
    ...
    public static JsonArray parseArray(Reader pReader) {
        return fromJson(GSON, pReader, JsonArray.class);
    }
