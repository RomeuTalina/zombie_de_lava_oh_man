...
public class BlockStateData {
    ...
    private static Dynamic<?> create(String pName) {
        return new Dynamic<>(JavaOps.INSTANCE, Map.of("Name", pName)).convert(NbtOps.INSTANCE);
    }

    private static Dynamic<?> create(String pName, Map<String, String> pProperties) {
        return new Dynamic<>(JavaOps.INSTANCE, Map.of("Name", pName, "Properties", pProperties)).convert(NbtOps.INSTANCE);
    }

    private static void register(int pIndex, Dynamic<?> pDefaultData, Dynamic<?>... pOthers) {
        MAP[pIndex] = pDefaultData;
        int i = pIndex >> 4;
        if (BLOCK_DEFAULTS[i] == null) {
            BLOCK_DEFAULTS[i] = pDefaultData;
        }

        for (Dynamic<?> dynamic : pOthers) {
            String s = dynamic.get("Name").asString("");
            ID_BY_OLD_NAME.putIfAbsent(s, pIndex);
            ID_BY_OLD.put(dynamic, pIndex);
        }
    }
