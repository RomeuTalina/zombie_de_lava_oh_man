...
public class TextComponentStringifiedFlagsFix extends DataFix {
    public TextComponentStringifiedFlagsFix(Schema pOutputSchema) {
    ...
    private static <T> Dynamic<T> stringToBool(Dynamic<T> pData) {
        Optional<String> optional = pData.asString().result();
        return optional.isPresent() ? pData.createBoolean(Boolean.parseBoolean(optional.get())) : pData;
    }
}