...
public class SortedArraySet<T> extends AbstractSet<T> {
    ...
    private void grow(int pSize) {
        if (pSize > this.contents.length) {
            if (this.contents != ObjectArrays.DEFAULT_EMPTY_ARRAY) {
                pSize = Util.growByHalf(this.contents.length, pSize);
            } else if (pSize < 10) {
                pSize = 10;
            }
    ...
    public boolean equals(Object pOther) {
        if (this == pOther) {
            ...
            return pOther instanceof SortedArraySet<?> sortedarrayset && this.comparator.equals(sortedarrayset.comparator)
